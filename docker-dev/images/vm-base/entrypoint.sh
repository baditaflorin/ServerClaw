...
if [ -f "$PUBKEY_PATH" ]; then
    sudo cp "$PUBKEY_PATH" /home/ops/.ssh/authorized_keys
...
if [ -S /var/run/docker.sock ]; then
    ...
    if [ -n "$DOCKER_GID" ] && [ "$DOCKER_GID" != "0" ]; then
        sudo groupadd -g "$DOCKER_GID" dockerhost 2>/dev/null || true
        sudo usermod -aG dockerhost ops 2>/dev/null || true
    fi
    sudo usermod -aG root ops 2>/dev/null || true
fi
