#!/usr/bin/env python3
"""Resolve stable per-session workspace metadata for controller-side tooling."""
...
def resolve_session_workspace(
    *,
    ...
    requested_session_id = (
        session_id
        or os.environ.get("LV3_SESSION_ID", "").strip()
        or _stable_session_id(checkout_root)
    )
    ...
    if remote_workspace_base is not None:
        remote_workspace_root = str(
            remote_workspace_base / SESSION_BASE_DIRNAME / session_slug / "repo"
        )
