...
from validation_toolkit import (
    require_str, require_mapping, require_list, require_string_list,
    require_bool, require_int, require_identifier, require_http_url,
    require_semver, require_enum, require_path, optional,
)


def test_require_str():
    assert require_str("hello", "x") == "hello"
    try: require_str(123, "x"); assert False
    except ValueError: pass
    try: require_str("", "x"); assert False
    except ValueError: pass
    try: require_str("  ", "x"); assert False
    except ValueError: pass
...
def test_require_mapping():
    ...
    try: require_mapping([], "x"); assert False
    except ValueError: pass
    try: require_mapping("str", "x"); assert False
    except ValueError: pass


def test_require_list():
    assert require_list([1, 2], "x") == [1, 2]
    try: require_list({}, "x"); assert False
    except ValueError: pass
    try: require_list([1], "x", min_length=2); assert False
    except ValueError: pass


def test_require_string_list():
    assert require_string_list(["a", "b"], "x") == ["a", "b"]
    try: require_string_list([1, 2], "x"); assert False
    except ValueError: pass
...
def test_require_bool():
    ...
    try: require_bool(1, "x"); assert False
    except ValueError: pass
    try: require_bool("true", "x"); assert False
    except ValueError: pass


def test_require_int():
    assert require_int(42, "x") == 42
    try: require_int(True, "x"); assert False
    except ValueError: pass
    try: require_int("1", "x"); assert False
    except ValueError: pass
    try: require_int(5, "x", minimum=10); assert False
    except ValueError: pass
    try: require_int(15, "x", maximum=10); assert False
    except ValueError: pass
...
def test_require_identifier():
    ...
    try: require_identifier("Hello", "x"); assert False
    except ValueError: pass
    try: require_identifier("1abc", "x"); assert False
    except ValueError: pass
...
def test_require_http_url():
    ...
    try: require_http_url("ftp://example.com", "x"); assert False
    except ValueError: pass
...
def test_require_semver():
    ...
    try: require_semver("1.2", "x"); assert False
    except ValueError: pass


def test_require_enum():
    assert require_enum("a", "x", ["a", "b", "c"]) == "a"
    try: require_enum("d", "x", ["a", "b", "c"]); assert False
    except ValueError: pass


def test_require_path():
    assert require_path("/etc/foo", "x") == "/etc/foo"
    try: require_path("relative/path", "x"); assert False
    except ValueError: pass
