#!/usr/bin/env python3
"""Verify Harbor completes Authentik OIDC with a non-admin test identity."""
...
CURRENT_USER_API_PATH = "/api/v2.0/users/current"


class VerificationError(RuntimeError):
    """Raised when Harbor does not establish the expected least-privilege session."""


def verify_harbor_user(status: int, payload: Any, *, username: str) -> dict[str, Any]:
    if status != 200 or not isinstance(payload, dict):
        raise VerificationError("Harbor did not return an authenticated user profile")
    if payload.get("username") != username:
        raise VerificationError("Harbor authenticated a different test identity")
    if payload.get("sysadmin_flag") is not False:
        raise VerificationError("Harbor test identity is not confirmed non-admin")
    if payload.get("admin_role_in_auth") is True:
        raise VerificationError("Harbor Authentik claims grant the test identity administrator access")
...
def verify_login(
    *,
    ...
    base_url = f"https://registry.{domain}"
    app_host = f"registry.{domain}"
    ...
    try:
        with sync_playwright() as playwright:
            if root_ca_file is None:
                ...
                temporary_profile = tempfile.TemporaryDirectory(prefix="harbor-authentik-e2e-")
            ...
            try:
                page.goto(f"{base_url}/harbor/projects", wait_until="domcontentloaded", timeout=timeout_ms)
                if urlparse(page.url).hostname == app_host:
                    login_link = page.get_by_role("link", name=re.compile(r"LOGIN WITH Authentik", re.IGNORECASE))
                    deadline = time.monotonic() + timeout_ms / 1000
                    selected_link = None
                    while time.monotonic() < deadline and selected_link is None:
                        for index in range(login_link.count()):
                            candidate = login_link.nth(index)
                    ...
                    if selected_link is None:
                        raise VerificationError("Harbor did not render its Authentik sign-in option")
                    selected_link.click(timeout=timeout_ms)

                if urlparse(page.url).hostname != identity_host:
                    raise VerificationError("Harbor did not redirect a fresh session to Authentik")
            ...
            if profile_result is None:
                raise VerificationError("Harbor did not establish an authenticated browser session")
            verified = verify_harbor_user(
