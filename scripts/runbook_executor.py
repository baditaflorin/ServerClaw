...
def validate_runbook_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    automation = require_mapping(payload.get("automation") or {}, "runbook.automation")
...
class RunbookRegistry:
    ...
    def resolve(self, identifier: str) -> dict[str, Any]:
        ...
        if candidate.exists():
            return validate_runbook_payload(parse_runbook_payload(candidate))

        for path in self._iter_candidates():
            if path.stem == identifier:
                return validate_runbook_payload(parse_runbook_payload(path))
            try:
                payload = parse_runbook_payload(path)
            except ValueError:
                continue
            if payload.get("id") == identifier:
                return validate_runbook_payload(payload)
...
class RunbookExecutor:
    ...
    def preview(self, runbook_ref: str, params: dict[str, Any]) -> dict[str, Any]:
        runbook = self.registry.resolve(runbook_ref)
        step_results: dict[str, Any] = {}
        rendered_steps = []
        run_record = {
            "run_id": "preview",
    ...
    def execute(self, runbook_ref: str, params: dict[str, Any], *, actor_id: str = "operator:lv3-cli") -> dict[str, Any]:
        runbook = self.registry.resolve(runbook_ref)
        run_record = {
        ...
        while True:
            ...
            if action == "rollback":
                ...
                if rollback_workflow:
                    ...
                    try:
                        self.workflow_runner.run_workflow(rollback_workflow, rollback_payload, timeout_seconds=step["timeout_seconds"])
                    except Exception as exc:  # noqa: BLE001
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        executor = RunbookExecutor(repo_root=repo_root, workflow_runner=default_runner(repo_root))
        if args.action == "execute":
            params = parse_params(args.param)
            if args.dry_run:
                print(json.dumps(executor.preview(args.runbook, params), indent=2, sort_keys=True))
                return 0
            print(json.dumps(executor.execute(args.runbook, params, actor_id=args.actor_id), indent=2, sort_keys=True))
            return 0
        if args.action == "status":
            print(render_status(executor.status(args.run_id)))
            return 0
        if args.action == "approve":
            print(json.dumps(executor.resume(args.run_id, actor_id=args.actor_id), indent=2, sort_keys=True))
            return 0
        parser.print_help()
        return 1
    except Exception as exc:  # noqa: BLE001
