...
def validate_renovate_config() -> None:
    ...
    require(
        any(
            "roles" in "".join(manager.get("managerFilePatterns", []))
            and "defaults" in "".join(manager.get("managerFilePatterns", []))
    ...
    require(
        any(
            "versions" in "".join(manager.get("managerFilePatterns", []))
            and "stack" in "".join(manager.get("managerFilePatterns", []))
            for manager in custom_managers
        ),
...
def validate_workflow() -> None:
    ...
    require(
        "schedule" in trigger and "workflow_dispatch" in trigger,
        "Renovate workflow must support schedule and manual dispatch",
    )
    ...
    require(
        "scripts/renovate_runtime_token.py create" in workflow_text,
        "Renovate workflow must mint a short-lived runtime token",
    )
    require(
        "scripts/renovate_runtime_token.py cleanup" in workflow_text,
        "Renovate workflow must revoke the short-lived runtime token",
    )
    require(
        "RENOVATE_BOOTSTRAP_ENV" in workflow_text,
        "Renovate workflow must source the mounted OpenBao-rendered credential bundle",
    )
    require(
        "-e RENOVATE_GIT_AUTHOR \\" in workflow_text,
        "Renovate workflow must pass the global git author into the runtime container",
    )
    require(
        "-e RENOVATE_REQUIRE_CONFIG \\" in workflow_text,
        "Renovate workflow must force optional config handling for branch-local validation",
    )
    require(
        "-e RENOVATE_ONBOARDING \\" in workflow_text,
        "Renovate workflow must disable onboarding during branch-local validation",
    )
    require(
    ...
    require(
        'current_container_id="${HOSTNAME:-$(hostname)}"' in workflow_text,
        "Renovate workflow must inspect the active job container to recover its workspace host path",
    )
    require(
        "/var/run/lv3/renovate" in workflow_text,
        "Renovate workflow must discover the host-side Renovate credential directory from the runner mount",
    )
    require(
        ".tmp/docker-bin.path" in workflow_text,
        "Renovate workflow must persist the discovered Docker executable path for later steps",
    )
    require(
        ".tmp/workspace-host.path" in workflow_text,
        "Renovate workflow must persist the discovered workspace host path for later steps",
    )
    require(
        ".tmp/bootstrap-host.path" in workflow_text,
        "Renovate workflow must persist the discovered credential host path for later steps",
    )
    require(
        "/var/run/lv3/renovate:ro" in workflow_text,
        "Renovate workflow must mount the Renovate credential bundle read-only",
    )
