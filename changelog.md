...
- Operational fixes: OpenBao unseal watcher, Keycloak VM correction, oauth2-proxy internal URL, dozzle-agent healthcheck, ServerClaw OIDC login, hairpin NAT extra_hosts, Plausible OIDC removal (CE v3.x uses oauth2-proxy only)
- removed all PatternFly v5 CSS framework classes from ops portal templates; portal now uses only custom portal.css classes; added repowise to service-capability-catalog and dependency-graph
- adds repowise semantic code search — local corpus builder, Ollama nomic-embed-text embeddings in Qdrant, FastAPI /search endpoint with language and document_kind filters; Ansible role repowise_runtime deploys on docker-runtime-lv3
...
- [2026 (441 releases)](docs/release-notes/index/2026.md)
