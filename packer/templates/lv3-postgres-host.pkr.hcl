...
source "proxmox-clone" "lv3_postgres_host" {
  ...
  vm_id                    = var.postgres_host_vmid
  vm_name                  = var.postgres_host_name
  template_name            = var.postgres_host_name
  template_description     = "LV3 PostgreSQL host template layered on lv3-debian-base"
...
build {
  sources = ["source.proxmox-clone.lv3_postgres_host"]
  ...
  provisioner "shell" {
    ...
    scripts = [
      "${path.root}/../scripts/postgres-install.sh",
    ]
  }
}
