...
- update nginx vhost `ops.example.com` — switch from static file serving to proxy to port 8092
...
- `https://ops.example.com` renders the interactive portal (requires Keycloak login)
...
- Log in at `ops.example.com` with Keycloak credentials; portal renders without JavaScript errors
...
- `ops.example.com/health` returns HTTP 200 (unauthenticated health endpoint for the portal itself)

## Merge Criteria

- Portal is deployed and accessible at `ops.example.com` behind Keycloak OIDC
...
- Live cutover completed on `2026-03-24`; `ops.example.com` now serves the interactive runtime through the OIDC-protected nginx edge with the corrected `8092` host-port contract
