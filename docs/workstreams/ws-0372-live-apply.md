...
- Status: merged
- Included In Repo Version: `0.178.145`
- Included In Platform Version: `0.178.145`
...
- The final closeout updates the protected integration surfaces only at merge time: `VERSION`, `RELEASE.md`, `docs/release-notes/0.178.145.md`, `README.md`, `versions/stack.yaml`, the archived workstream shard, and the integrated receipts now all agree on repo and platform version `0.178.145`.

## Verification

- Focused ADR 0372 regression coverage passed on the latest exact-main candidate after rebasing onto the ws-0382 mainline tip:
  ...
  returned `54 passed in 3.13s`; evidence:
  `receipts/live-applies/evidence/2026-04-15-ws-0372-mainline-pytest-r2-0.178.144.txt`.
...
- `ALLOW_IN_PLACE_MUTATION=true make live-apply-service service=directus env=production` completed successfully from the latest realistic mainline candidate after ws-0382, with a clean Directus replay across `docker-runtime`, `postgres`, `nginx`, and `localhost`; evidence:
  `receipts/live-applies/evidence/2026-04-15-ws-0372-mainline-directus-live-apply-r3-0.178.144.txt`.
- The governed replay verified the internal Directus health endpoint, `server/ping`, `server/specs/oas`, the schema bootstrap path, the public `https://data.<platform-domain>/server/health` endpoint, and the token-backed publication verification flow. The wrapper also completed its post-apply restic trigger and recorded `receipts/restic-backups/20260415T074137Z.json`, which updated `receipts/restic-snapshots-latest.json`.
- The integrated `0.178.145` candidate also passes the final repo gates, live-apply receipt validation, `git diff --check`, build-server reachability, and `make remote-validate`; the matching evidence is recorded under the `2026-04-15-ws-0372-mainline-final-*.txt` transcripts in `receipts/live-applies/evidence/`.
