...
- branch-side validation is now current against that `0.177.123` baseline: `uv run --with pytest --with pyyaml python -m pytest -q tests/test_livekit_playbook.py tests/test_livekit_runtime_role.py tests/test_linux_guest_firewall_role.py tests/test_ansible_execution_scopes.py tests/test_dependency_wave_apply.py` passed with `27 passed`, `make syntax-check-livekit` passed, `scripts/validate_repo.sh agent-standards` passed, and `uv run --with pyyaml python scripts/ansible_scope_runner.py validate` passed
