...
- this branch intentionally still leaves the protected integration files untouched: `README.md`, `VERSION`, `changelog.md`, `docs/release-notes/`, and `versions/stack.yaml`
- the merge-to-main step must update those protected truth surfaces, map `control_plane_recovery` to receipt `2026-03-28-adr-0231-local-secret-delivery-live-apply`, regenerate generated release artifacts, and then rerun the repo validation gates from `main`
...
- once this workstream stays in `live_applied`, `./scripts/validate_repo.sh workstream-surfaces` and therefore `make validate` will fail on this branch by design because terminal workstreams are no longer valid branch owners; rerun the full validation sweep after integrating onto `main`
