...
- Worktree: `/Users/live/Documents/GITHUB_PROJECTS/proxmox_florin_server/.worktrees/ws-0274-mainline-v3`
- Owner: codex
...
- `uv run --with pytest pytest tests/test_repo_deploy_image_cache.py tests/test_coolify_playbook.py tests/test_coolify_runtime_role.py tests/test_docker_publication_assurance.py` returned `27 passed in 24.60s`.
...
- `make live-apply-service service=coolify env=production EXTRA_ARGS='-e bypass_promotion=true'` completed successfully from the refreshed latest-main candidate with final recap `coolify-lv3 : ok=161 changed=11 unreachable=0 failed=0 skipped=13 rescued=0 ignored=0`, `nginx-lv3 : ok=81 changed=4 unreachable=0 failed=0 skipped=16 rescued=0 ignored=0`, and `proxmox_florin : ok=273 changed=5 unreachable=0 failed=0 skipped=134 rescued=0 ignored=0`.
- Direct SSH verification on `coolify-lv3` confirmed `sudo /usr/local/bin/lv3-docker-publication-assurance ...` returned `"ok": true` with summary `"docker publication contract is satisfied"`.
- Direct SSH verification on `coolify-lv3` confirmed `/opt/repo-deploy-image-cache/warm-status.json` reported `result: pass`, `successful_pulls: 5`, `failed_pulls: 0`, `changed_count: 0`, `plan_generated_at: 2026-03-30T12:21:46Z`, and `warmed_at: 2026-03-30T12:21:52Z`, while `lv3-repo-deploy-image-cache.timer` remained `enabled` and `active`.
- Direct SSH verification on `coolify-lv3` confirmed `docker image inspect` resolved all five approved warmed refs locally.
- Public verification confirmed `https://coolify.lv3.org/` and `https://coolify.lv3.org/login` both returned the expected `302` redirect to `/oauth2/sign_in` with `x-robots-tag: noindex, nofollow`.
- Public verification confirmed `https://docs.lv3.org/robots.txt` and `https://changelog.lv3.org/robots.txt` still returned `Disallow: /` after the exact-main replay.
- Direct host verification confirmed `host=Debian-trixie-latest-amd64-base`, `kernel=6.17.13-2-pve`, `pve=pve-manager/9.1.6/71482d1833ded40a (running kernel: 6.17.13-2-pve)`, and `vm170=running`.
...
- This mainline candidate adds the approved deployment profile catalog, the `repo_deploy_image_cache` role, the guest-local warm-plan and receipt contract, the scheduled timer-driven refresh surface, and the Coolify bootstrap/publication hardening required to keep the replay self-healing.
...
- none after the `0.177.102` exact-main integration and canonical receipt refresh
