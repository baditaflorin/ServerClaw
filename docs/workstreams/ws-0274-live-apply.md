...
- `make live-apply-service service=coolify env=production EXTRA_ARGS='-e bypass_promotion=true'` completed successfully from the refreshed latest-main candidate with final recap `coolify-lv3 : ok=161 changed=10 unreachable=0 failed=0 skipped=14 rescued=0 ignored=0`, `nginx-lv3 : ok=81 changed=4 unreachable=0 failed=0 skipped=16 rescued=0 ignored=0`, and `proxmox_florin : ok=273 changed=5 unreachable=0 failed=0 skipped=134 rescued=0 ignored=0`.
- Direct SSH verification on `coolify-lv3` confirmed `sudo /usr/local/bin/lv3-docker-publication-assurance ...` returned `"ok": true` with summary `"docker publication contract is satisfied"` after the helper was hardened to tolerate hosts that ship `nftables` without an `iptables` binary.
