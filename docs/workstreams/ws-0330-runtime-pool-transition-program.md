...
- `runtime-control-lv3` is declared as the fixed-capacity control-plane guest
...
- `playbooks/runtime-control-pool.yml` and
  ...
  the legacy control-plane copies on `docker-runtime-lv3`.
- the control-plane service metadata, health probes, proxy contracts, and
  runbooks now point at `runtime-control-lv3` for the moved `api_gateway`,
...
- verify the actual `runtime-control-lv3` guest exists live before retiring the
  legacy control-plane copies on `docker-runtime-lv3`
