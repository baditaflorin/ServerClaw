...
- Title: Deploy Plausible Analytics on `docker-runtime`, publish it at `analytics.example.com`, and verify privacy-first page tracking end to end
- Status: live_applied
...
- Worktree: `/Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.worktrees/ws-0283-live-apply`
- Owner: codex
...
- Conflicts With: none
- Shared Surfaces: `docs/adr/0283`, `docs/workstreams/ws-0283-live-apply.md`, `docs/runbooks/configure-plausible.md`, `inventory/host_vars/proxmox-host.yml`, `inventory/group_vars/platform.yml`, `playbooks/plausible.yml`, `playbooks/services/plausible.yml`, `roles/plausible_runtime/`, `roles/nginx_edge_publication/`, `collections/ansible_collections/lv3/platform/plugins/filter/service_topology.py`, `config/*catalog*.json`, `config/ansible-execution-scopes.yaml`, `receipts/image-scans/`, `receipts/live-applies/`, `tests/`

## Scope

- deploy Plausible Community Edition on `docker-runtime` with repo-managed PostgreSQL, ClickHouse, and OpenBao-backed runtime secrets
- publish `analytics.example.com` through the shared NGINX edge with dashboard access gated by the existing edge OIDC flow while tracker and health endpoints remain public
...
- Plausible Community Edition stack running on `docker-runtime`
- public hostname `analytics.example.com`
- public tracker script and event endpoints at `https://analytics.example.com/js/` and `https://analytics.example.com/api/event`
...
- `docker-runtime` and `nginx-edge` are shared live surfaces, so replay must use the governed service wrapper and a documented narrow in-place exception if ADR 0191 blocks the default path
...
analytics layer on `docker-runtime`, publishing `analytics.example.com`
...
- `2b35ffca6` hardened the fresh-worktree replay path by generating the shared
  edge portal artifacts before publication, admitting `analytics.example.com` into
...
- `HETZNER_DNS_API_TOKEN=... make converge-plausible` completed successfully
  with final recap
  `docker-runtime : ok=145 changed=4 unreachable=0 failed=0 skipped=21 rescued=0 ignored=0`,
...
- Fresh public verification confirmed
  `https://analytics.example.com/api/system/health/ready` returned the same ready
  payload, the `nginx.example.com` page now includes the
  `https://analytics.example.com/js/script.js` tracker snippet, the dashboard
  remains OIDC-gated with `HTTP/2 302` plus `x-robots-tag: noindex, nofollow`,
  and `analytics.example.com` resolves publicly to `203.0.113.1`.
- A public end-to-end analytics smoke posted to
  `https://analytics.example.com/api/event` with `202 Accepted`, then confirmed the
...
- The exact-main replay from committed source `f8bd6088817ecd676f9c59f17f52e9cf0dd20b56`
  ...
  `docker-runtime : ok=145 changed=5 unreachable=0 failed=0 skipped=21 rescued=0 ignored=0`,
  `localhost : ok=18 changed=0 unreachable=0 failed=0 skipped=3 rescued=0 ignored=0`,
  and
  `nginx-edge : ok=39 changed=5 unreachable=0 failed=0 skipped=7 rescued=0 ignored=0`,
  preserved in
...
- A fresh Proxmox host-state probe preserved in
  ...
  confirmed the server still reports hostname `proxmox-host`,
  `pve-manager/9.1.6/71482d1833ded40a (running kernel: 6.17.13-2-pve)`, and
  running VMs `110` (`nginx-edge`) plus `120` (`docker-runtime`).
...
- A fresh exact-main public verification preserved in
  ...
  reconfirmed the ready payload, the `nginx.example.com` tracker snippet, the OIDC
  dashboard redirect with `x-robots-tag: noindex, nofollow`, and public DNS for
  `analytics.example.com -> 203.0.113.1`.
- A fresh exact-main public event smoke preserved in
  ...
  posted `202 Accepted` to `https://analytics.example.com/api/event` and confirmed
