...
- write `config/service-completeness.json` — tracks completeness status for all current services; marks pre-existing services as `grandfathered: true`
- update `config/validation-gate.json` — add `service-completeness` check (runs `validate_service_completeness.py`; grandfathered items suppressed until 2026-09-23)
...
- `lv3 scaffold new-service --name test-service --vm docker-runtime-lv3 --depends-on postgres,keycloak --type compose` generates all 15 artifacts in the correct directories
...
- Run `lv3 scaffold new-service --name demo-service --vm docker-runtime-lv3 --depends-on postgres --type compose`
- Verify all 15 expected files/patches are created
- Run `lv3 validate --service demo-service` → all 15 items present
...
- Grandfathering in `config/validation-gate.json` should be implemented as a `suppressed_until` date field per check, not a global suppression; this allows individual items to graduate from grandfathered status independently as they are fixed
