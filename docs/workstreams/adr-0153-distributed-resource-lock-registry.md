...
- The first 2026-03-25 retry reached `proxmox-host`, `postgres`, and `docker-runtime` but failed when the local OpenBao API on `docker-runtime` answered `503` with `sealed: true`.
...
- Recovery also required one manual Docker daemon restart on `docker-runtime` to restore the missing `nat` `DOCKER` chain and one manual OpenBao unseal using the existing controller-local recovery material after an interrupted replay left the runtime half-unsealed.
