...
`0.177.60`, `0.177.61`, `0.177.62`, and `0.177.63`, rerun the guarded
production replay from the final `0.177.63` base, record fresh exact-main
evidence, and cut repository release `0.177.64` without changing the live
platform baseline from `0.130.44`.
...
- `git fetch origin --prune` confirmed the final integration base was
  `origin/main` commit `7c71a5c8` with repository version `0.177.63`
- `make immutable-guest-replacement-plan service=nomad` confirmed ADR 0191
  still classifies `monitoring-lv3` as `edge_and_stateful` with validation mode
...
- `make live-apply-service service=nomad env=production ALLOW_IN_PLACE_MUTATION=true`
  completed successfully from the rebased `0.177.63` worktree with final recap
  `docker-build-lv3 ok=83 changed=5 failed=0`,
...
- controller-side Nomad verification returned leader `"10.10.10.40:4647"` and
  nodes `docker-runtime-lv3 ready runtime` plus `docker-build-lv3 ready build`
- guest-side smoke verification returned `lv3-nomad-smoke-service` as
  ...
  `docker-runtime-lv3` recorded `2026-03-29T01:57:54+00:00`
- `LV3_SKIP_OUTLINE_SYNC=1 uv run --with pyyaml python scripts/release_manager.py --bump patch --platform-impact "release 0.177.64 records the verified ADR 0232 Nomad replay from the latest origin/main while the live platform baseline remains 0.130.44"` prepared release `0.177.64`
...
- release `0.177.64` records ADR 0232 on `main` and promotes canonical receipt
