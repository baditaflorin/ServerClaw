...
- cut release `0.177.44` without a new platform-version bump because ADR 0229 was already live before the concurrent ADR 0225 mainline release
- refresh generated canonical-truth surfaces and re-run the repository validation gate before pushing `origin/main`

## Result

- Release `0.177.44` integrates ADR 0229 into `origin/main` by carrying forward the latest-main verification replay, the private Gitea push-gate proof, and workflow run `36` on runner `docker-build-lv3`.
