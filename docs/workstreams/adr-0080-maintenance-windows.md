...
- Conflicts With: none
- Shared Surfaces: NATS KV store (`maintenance-windows` bucket), `config/command-catalog.json`, Uptime Kuma API, Grafana API, `Makefile`
...
- open a maintenance window for grafana; manually trigger the observation loop; verify Mattermost `#platform-findings` does NOT receive a grafana finding
- allow the TTL to expire; verify the observation loop resumes notifying for grafana
- force-close a window; verify `maintenance.force_closed` event appears in Loki
- security findings (certificate expiry) are NOT suppressed by a maintenance window
...
- auto-close via NATS KV TTL is verified working
