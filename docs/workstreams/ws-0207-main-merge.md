...
- prove the integrated branch through local validation, the authoritative build-server `remote-validate` pass on the merged `0.177.38` tree, the final `0.177.42` `remote-validate` fallback pass, worker post-merge gate replay, and a governed live Hetzner DNS reconcile
