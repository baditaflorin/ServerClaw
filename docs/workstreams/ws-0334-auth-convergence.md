...
- the fresh exact-main replay `make converge-open-webui env=production` completed successfully with recap `docker-runtime-lv3 ok=278 changed=5 failed=0` and `proxmox_florin ok=40 changed=4 failed=0`; the full transcript is preserved in `receipts/live-applies/evidence/2026-04-04-ws-0334-mainline-converge-open-webui-r1.txt`
- direct post-apply verification remained green after that replay: Keycloak discovery returned `HTTP 200`, the Open WebUI OIDC login endpoint returned `HTTP 302` into the `open-webui` client on `sso.lv3.org`, and the break-glass admin sign-in returned `HTTP 200` for `ops@lv3.org`; the proof is preserved in `receipts/live-applies/evidence/2026-04-04-ws-0334-mainline-direct-verification-r1.txt`
