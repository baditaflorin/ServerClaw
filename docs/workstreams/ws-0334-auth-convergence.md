...
- no attempt in this workstream to move `uptime.example.com` behind shared edge auth because the repo-managed Uptime Kuma Socket.IO client still depends on a separate management path that is not yet provisioned
- no broader host-placement migration for Keycloak or the mail platform; this workstream follows the observed live placement on `docker-runtime` so the Open WebUI client can converge against the current identity runtime
...
- add playbook and topology tests that assert the top-level Keycloak converge surface and service map target `docker-runtime`
...
- production `make converge-open-webui` passed on 2026-04-03 with recap `docker-runtime ok=273 changed=3 failed=0` and `proxmox-host ok=40 changed=4 failed=0`
- public SSO discovery now returns `HTTP 200` from `https://sso.example.com/realms/lv3/.well-known/openid-configuration`
- Open WebUI now redirects `http://100.64.0.1:8008/oauth/oidc/login` to the Keycloak `open-webui` client on `sso.example.com`
- the break-glass login path still works at `http://100.64.0.1:8008/api/v1/auths/signin` for `ops@example.com`
...
## Exact-Main Verification Follow-Up

- on `2026-04-04`, an isolated exact-main worktree from `origin/main@20a66bbf0` (`repo 0.178.3`, `platform 0.130.98`) exposed one remaining repository-contract gap: `scripts/generate_platform_vars.py` did not emit `platform_service_topology.keycloak.urls.internal` or the matching internal port even though host vars, runtime defaults, and ADR 0341 tests all depended on that generated value
- the generator now derives the internal Keycloak URL and port from the declared edge upstream, the generated `inventory/group_vars/platform.yml` matches that contract again, and `scripts/topology-snapshot.json` has been refreshed so `agent-standards` no longer warns about stale topology generation state
- exact-main validation passed after the fix: `scripts/validate_repo.sh generated-vars data-models`, `make syntax-check-keycloak`, `make syntax-check-open-webui`, and `uv run --with pytest pytest tests/test_generate_platform_vars.py tests/test_keycloak_playbook.py tests/test_keycloak_runtime_role.py tests/test_open_webui_playbook.py tests/test_open_webui_runtime_role.py tests/test_docker_runtime_role.py tests/test_postgres_vm_role.py` returned `113 passed`
- the fresh exact-main replay `make converge-open-webui env=production` completed successfully with recap `docker-runtime ok=278 changed=5 failed=0` and `proxmox-host ok=40 changed=4 failed=0`; the full transcript is preserved in `receipts/live-applies/evidence/2026-04-04-ws-0334-mainline-converge-open-webui-r1.txt`
- direct post-apply verification remained green after that replay: Keycloak discovery returned `HTTP 200`, the Open WebUI OIDC login endpoint returned `HTTP 302` into the `open-webui` client on `sso.example.com`, and the break-glass admin sign-in returned `HTTP 200` for `ops@example.com`; the proof is preserved in `receipts/live-applies/evidence/2026-04-04-ws-0334-mainline-direct-verification-r1.txt`
- the exact-main integration pass also exposed a repo-automation gap outside the live platform surface: `scripts/atlas_schema.py` still assumed loopback-only binds were reachable from sibling Docker containers and that every runner imported `docker.errors`; the helper now binds Atlas SSH tunnels and ephemeral lint PostgreSQL ports on `0.0.0.0` for Linux host-gateway reachability, falls back to exception class-name matching when `docker.errors` is absent, and covers both cases in `tests/test_atlas_schema.py`
- the repaired automation path is verified locally by `uv run --with pytest pytest tests/test_atlas_schema.py` (`27 passed`) and `make atlas-lint`, with the direct Atlas lint proof preserved in `receipts/live-applies/evidence/2026-04-04-ws-0334-mainline-atlas-lint-r1.txt`
- no new platform version bump is required because ADR 0341 first became true on `0.130.98`; this follow-up repairs the generated repository contract and re-verifies the already-live behavior on current exact-main truth rather than introducing a new platform capability
