...
- ADR: [ADR 0198](/Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/docs/adr/0198-qdrant-vector-search-semantic-rag.md)
- Title: Semantic retrieval for platform RAG using Qdrant and local Ollama embeddings
- Status: merged
- Branch: `codex/ws-0198-live-apply`
- Worktree: `/Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.worktrees/ws-0198-live-apply`
- Owner: codex
...
- repair degraded vector collections during governed live apply without forcing a synchronous full corpus rebuild on `docker-runtime`
...
- the integrated mainline release and its post-apply truth are recorded on `main` without a follow-up merge-only cleanup step
...
- `docker-runtime` serves vector retrieval from a semantic Ollama-backed collection and repairs legacy dimension drift during managed live apply
...
- The synchronized latest-main replay on 2026-03-28 ran from `origin/main` commit `b2756a4f` through `make live-apply-service service=rag-context env=production`, completed with `docker-runtime : ok=96 changed=3 unreachable=0 failed=0 skipped=16` and `proxmox-host : ok=36 changed=4 unreachable=0 failed=0 skipped=14`, and repaired a degraded legacy `384`-dimension collection with a bounded controller-side semantic seed rebuild before the final vector assertion.
- The committed `0.177.26` main-candidate replay on 2026-03-28 ran from commit `4e492732` through `make live-apply-service service=rag-context env=production ALLOW_IN_PLACE_MUTATION=true`, completed with `docker-runtime : ok=94 changed=4 unreachable=0 failed=0 skipped=19` and `proxmox-host : ok=36 changed=4 unreachable=0 failed=0 skipped=14`, and preserved the healthy semantic vector collection without needing the repair step.
...
- Production live apply now succeeds from committed `main` through the governed `rag-context` service entrypoint, and the final 2026-03-28 replay verified that healthy vector retrieval is preserved without forcing a rebuild.
- A full repo-grounded rebuild still belongs to an explicit maintenance action because the expanded mirrored corpus exceeds the safe synchronous Ollama budget on `docker-runtime`.
