...
- Worktree: `/Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.worktrees/ws-0199-live-apply`
- Owner: codex
...
- Conflicts With: none
- Shared Surfaces: `inventory/host_vars/proxmox-host.yml`, `config/{service-capability-catalog,subdomain-catalog,health-probe-catalog,image-catalog,secret-catalog,controller-local-secrets,api-gateway-catalog,dependency-graph,slo-catalog,data-catalog,service-completeness,service-redundancy-catalog}.json`, `collections/ansible_collections/lv3/platform/roles/{outline_runtime,outline_postgres,keycloak_runtime}`, `playbooks/outline.yml`, `playbooks/services/outline.yml`, `scripts/sync_docs_to_outline.py`, `scripts/release_manager.py`, `docs/runbooks/configure-outline.md`

## Scope

- deploy Outline on `docker-runtime` with Postgres, Redis, MinIO, and Keycloak OIDC
- publish `wiki.example.com` through the shared NGINX edge
...
- `wiki.example.com` is healthy, published through the shared edge, and the managed Outline surface contains exactly the five governed top-level collections: `ADRs`, `Agent Findings`, `Architecture`, `Incident Postmortems`, and `Runbooks`
