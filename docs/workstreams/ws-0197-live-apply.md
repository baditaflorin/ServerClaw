...
- Shared Surfaces: `config/service-capability-catalog.json`, `config/health-probe-catalog.json`, `config/subdomain-catalog.json`, `config/subdomain-exposure-registry.json`, `config/workflow-catalog.json`, `config/controller-local-secrets.json`, `config/secret-catalog.json`, `config/image-catalog.json`, `inventory/host_vars/proxmox-host.yml`, `scripts/api_gateway/main.py`, `config/agent-tool-registry.json`, `platform/dify-workflows/`, `docs/runbooks/configure-dify.md`, `receipts/live-applies/`

## Scope

- replay ADR 0197 from the latest `origin/main` in an isolated worktree and branch suitable for concurrent agent work
- deploy Dify on `docker-runtime` behind `agents.example.com` using repo-managed Ansible playbooks and pinned container images
...
- `curl -fsS -D - https://agents.example.com/healthz`
- `make converge-api-gateway`
- `curl -fsS -D - https://api.example.com/v1/health`
- `curl -sS -X POST https://api.example.com/v1/dify-tools/get-platform-status -H "Content-Type: application/json" -H "X-LV3-Dify-Api-Key: $(cat /Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/dify/tools-api-key.txt)" -d '{}'`
- `curl -fsS -D - http://127.0.0.1:18094/healthz`
- `uv run --with requests --with pyyaml python3 scripts/dify_smoke.py --base-url http://127.0.0.1:18094 --admin-email operator@example.com --admin-password-file /Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/dify/admin-password.txt --init-password-file /Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/dify/init-password.txt --tools-api-key-file /Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/dify/tools-api-key.txt --export-path platform/dify-workflows/lv3-dify-smoke.yml`

## Outcome

- The rebased current-main replay completed successfully on top of release `0.177.46`, with `make converge-dify` finishing `docker-runtime ok=111 changed=0 failed=0`, `nginx-edge ok=38 changed=4 failed=0`, and `postgres ok=43 changed=4 failed=0`, followed by `make converge-api-gateway` finishing `docker-runtime ok=234 changed=106 failed=0`.
- Public publication is now live and verified: `dig +short agents.example.com` returned `203.0.113.1`, `curl -fsS -D - https://agents.example.com/healthz` returned `HTTP/2 200`, and the Dify setup endpoint still reports `step=finished`.
- The platform API gateway replay restored packaged runbook compatibility while preserving the current ADR 0209 use-case service design, and the governed bridge now returns `HTTP/2 200` from `https://api.example.com/v1/dify-tools/get-platform-status` after the replay.
