...
- The validator implementation was already present on exact `origin/main` at `b980bb92e62c10c21678317120b07ef0ae98dea9`; no code change to `scripts/validate_public_entrypoints.py` or the gate wiring was required for ADR 0336 itself.
...
- Focused regression coverage passed on the exact-main worktree: `uv run --with pytest --with pyyaml python3 -m pytest -q tests/test_validate_public_entrypoints.py tests/test_validate_repo_cache.py tests/test_generate_discovery_artifacts.py` returned `29 passed in 2.84s`, preserved in `receipts/live-applies/evidence/2026-04-04-ws-0336-targeted-tests-r1.txt`.
...
- `make remote-validate` exercised the remote-wrapper path end to end: the remote command failed, the wrapper fell back locally, and the fallback runner passed `agent-standards`, `alert-rule-validation`, `atlas-lint`, `dependency-graph`, `iac-policy-scan`, `policy-validation`, and `type-check`, while `workstream-surfaces` failed because the branch now maps to terminal history and `ansible-syntax` plus `schema-validation` timed out after `901.21s` and `601.47s`. That result is preserved verbatim in `receipts/live-applies/evidence/2026-04-04-ws-0336-remote-validate-r1.txt`.
...
- The validator already ships on `origin/main`; this workstream exists to make that state auditable from a dedicated branch/worktree even after `origin/main` advanced to `b980bb92e62c10c21678317120b07ef0ae98dea9`.
