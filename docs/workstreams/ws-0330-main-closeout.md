...
- This closeout intentionally leaves `README.md`, `changelog.md`, and `versions/stack.yaml` untouched on the branch because ADR 0330 is being verified as repo-only exact-main truth without cutting a new release.
- The current realistic integrated version remains `0.178.3`; the ADRs become `Implemented` now, but `Implemented In Repo Version` stays `not yet` until a later numbered release includes this exact-main closeout.
