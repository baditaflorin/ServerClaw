...
- Title: Live apply the systemd service, timer, and path baseline for host-resident control loops on `proxmox-host`
- Status: live_applied
...
- Worktree: `/Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.worktrees/ws-0226-live-apply`
- Owner: codex
...
- `proxmox-host` host-only `systemd` units:
...
- `make configure-host-control-loops` replayed cleanly from `b56e6dee15d4caca0e326235bdc7403a4b426e49` with `proxmox-host ok=33 changed=2 failed=0`; the replay updated the service unit and cleared a stale failed state before reasserting the timer and path units.
- `sudo systemctl show` on `proxmox-host` reported `lv3-host-control-loop-reconcile.service Result=success ActiveState=inactive`, the timer as `active/waiting` with `NextElapseUSecRealtime=Sat 2026-03-28 18:00:59 CET` and `LastTriggerUSec=Sat 2026-03-28 17:31:04 CET`, and the path unit as `active/waiting` with `Triggers=lv3-host-control-loop-reconcile.service`.
...
- ADR 0226 is live on `proxmox-host` with repo-managed `systemd` service, timer, and path units plus durable state under `/var/lib/lv3-host-control-loops/`.
