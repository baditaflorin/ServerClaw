...
- `make remote-validate` passed twice across the final merge: first as the authoritative build-server manifest on the merged `0.177.38` tree, then again on the merged `0.177.40` recut after `scripts/remote_exec.sh` fell back locally when build-server `rsync` could not set mtimes under `build/docs-portal/*`
