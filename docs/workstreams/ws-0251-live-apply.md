...
- The remaining merge step for this follow-up is to cut the next patch release on `main`, replay the merged exact-main OpenBao and Windmill verification from the latest `origin/main`, and then update the protected release plus canonical-truth surfaces together once the merged commit is what the live worker checkout and OpenBao runtime are actually running.
