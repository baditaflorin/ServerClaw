...
- integrate the first governed replay into the restore-verification recovery path for restored `docker-runtime-lv3`
...
- Initial full restore-verification replay from this worktree wrote `receipts/restore-verifications/2026-03-27-ws0190-attempt1.json`; it exposed two pre-existing restore-path issues outside ADR 0190 itself: the default 5-minute SSH wait was too short for restored guests, and `backup-lv3` currently has no PBS backup available
- Focused `docker-runtime-lv3` replays from this worktree wrote `receipts/restore-verifications/2026-03-27-ws0190-attempt2-qga.json` and the final `receipts/restore-verifications/2026-03-27.json`; the new Proxmox `qga` fallback allowed smoke tests and the ADR 0190 synthetic replay to execute end to end even without SSH
...
- the repository now carries a reusable synthetic replay harness plus the first governed replay catalog for restored `docker-runtime-lv3`
