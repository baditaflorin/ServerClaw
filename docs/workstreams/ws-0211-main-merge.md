...
- Worktree: `/Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.worktrees/ws-0211-main-merge`
- Owner: codex
...
- The merged-main `headscale` replay completed successfully with `localhost ok=16 changed=0 failed=0`, `nginx-edge ok=71 changed=3 failed=0`, and `proxmox-host ok=42 changed=0 failed=0`.
- The public health path `https://headscale.example.com/health` still returned `HTTP 200` after the merged-main replay, preserving the shared edge security headers and crawl policy.
