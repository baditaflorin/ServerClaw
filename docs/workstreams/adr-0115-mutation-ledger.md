...
- 2026-03-26 live apply verified the production Postgres ledger schema on platform version `0.130.20`, including the `audit_log` compatibility view, append-only trigger enforcement, and a live `execution.completed` row written from `docker-runtime`
...
- 2026-03-27 replay from the latest `origin/main` verified the Windmill runtime projection on `docker-runtime`: `/run/lv3-secrets/windmill/runtime.env` and `windmill-windmill_worker-1` both carried `LV3_LEDGER_DSN` and `LV3_LEDGER_NATS_URL`, and a fresh guest-side emit wrote `2|execution.completed|operator:ops|host|proxmox-host|ws0115-main-ledger-emit-20260327T044831Z` into `ledger.events`
