...
- repository implementation completed in `0.143.2`
...
- the live apply is still pending because this controller could not reach the intended tailnet path, and the public IPv4 currently lands in Hetzner rescue mode rather than the active Proxmox host

## Notes For The Next Assistant

- Start the live rollout from an execution surface that can reach `100.118.189.95` and the guest jump path; the current session could not.
