...
moves during closeout, cut the final `0.177.119` release from that exact-main
...
- `origin/main` then advanced again to commit `2411a7cd428e0eba17168aa5eed66f04c4ed48dd`, which already carried repository version `0.177.118` and platform version `0.130.77`, so the workstream was rebased a second time and recut as release `0.177.119` from that newer latest-main baseline.
...
- ADR 0305 is fully integrated on the dedicated exact-main branch and included
  in repository release `0.177.119`.
