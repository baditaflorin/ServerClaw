...
- Worktree: `/Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.worktrees/wiki-edge-cert-fix`
- Owner: codex
...
- restore the live publication for `wiki.example.com` and preserve `draw.example.com` in the same certificate and rendered edge config
...
- `curl -I https://wiki.example.com/_health`
...
- `python3 scripts/sync_docs_to_outline.py verify --repo-root /Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.worktrees/wiki-edge-cert-fix --base-url https://wiki.example.com --api-token-file /Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/outline/api-token.txt`
...
- the live repair replay restored `wiki.example.com` and `draw.example.com` to `/etc/nginx/sites-available/lv3-edge.conf`, expanded the shared certificate SANs to include both hostnames again, and returned `HTTP/2 200` for `https://wiki.example.com` and `https://wiki.example.com/_health`
