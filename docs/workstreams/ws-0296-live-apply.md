...
- Title: Provision `artifact-cache`, move the build host onto the dedicated cache plane, and record the exact live-apply proof from latest `origin/main`
- Status: live_applied
...
- Worktree: `/Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.worktrees/ws-0296-live-apply`
- Owner: codex
...
- VM `180` exists as `artifact-cache` at `10.10.10.80`
- `artifact-cache` serves the four private cache endpoints on TCP `5001-5004`
- `docker-build` consumes those dedicated endpoints and no longer runs the old local `artifact-cache-*` containers
...
- VM `180` now exists as `artifact-cache` on `10.10.10.80/24` with the pinned MAC address `BC:24:11:18:AC:80`, `4` vCPUs, `8192` MiB RAM, `160` GiB disk, and the declared internal-only firewall policy.
- The dedicated cache runtime is live on `artifact-cache`, with `artifact-cache-docker-hub`, `artifact-cache-ghcr`, `artifact-cache-plane`, and `artifact-cache-n8n` all serving TCP `5001-5004` and the repo-derived `51`-image warm set now materialized on the dedicated guest instead of the build host.
- `docker-build` is now a cache-plane consumer only: it retains `apt-cacher-ng` and `lv3-buildkitd`, no longer runs the old local `artifact-cache-*` containers, and now resolves Docker pulls through the dedicated cache endpoints.
...
- Broader runtime-host adoption beyond `docker-build` remains later phased work under the ADR, not a blocker for this live apply.
