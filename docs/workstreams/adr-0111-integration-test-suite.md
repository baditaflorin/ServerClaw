...
- When the build server has live credentials and network access, confirm `test_keycloak_issues_valid_jwt` obtains a real JWT from `sso.lv3.org`
...
- Live rollout of nightly schedule, Keycloak `test-runner@lv3.org`, and OpenBao credential storage remains a post-merge apply task from `main`
