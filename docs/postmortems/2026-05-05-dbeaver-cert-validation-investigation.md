...
**Answer**: **example.com** (not example.org)

- **example.com** = Private/production deployment domain (from `.local/identity.yml`)
- **example.org** = Would be the public fork domain (not in use here)
- **example.com** = Generic placeholder in committed catalogs for public GitHub reusability

The deployment uses example.com exclusively. The example.org domain would only appear if this repo were forked for a different organization.
