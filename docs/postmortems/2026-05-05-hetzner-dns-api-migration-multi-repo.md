...
3. **Supporting multiple concurrent deployments** (example.com private + 0fork.com public fork)

The DNS role is **shared across all deployments** but each deployment has:
- Different zone names (example.com vs 0fork.com)
...
But diverge in:
- **Zones**: `example.com` vs `0fork.com`
...
- **Validate across all deployments** — example.com and 0fork.com patterns
