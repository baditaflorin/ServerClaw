...
and streaming system. The architecture used a parent agent on `monitoring-lv3`
...
1. **Coverage overlap**: Every metric Netdata collected is also collected by
   ...
   `monitoring-lv3`. The Grafana dashboard `1860 - Node Exporter Full` provides
...
3. **Architectural alignment**: The platform observability stack is
   ...
   casing in catalogs, playbooks, and the nginx edge (realtime.lv3.org).

4. **Active browser dashboards**: The real-time browser dashboard at
   `realtime.lv3.org` was the most-cited reason to keep Netdata. Grafana's
...
| Inventory/config | 4 | `proxmox_florin.yml`, `platform.yml`, port assignments |
...
Run on `monitoring-lv3` and all guest VMs:
...
- `inventory/host_vars/proxmox_florin.yml` — removed `netdata_port` from port assignments, removed `lv3_service_topology.realtime` block
...
```bash
# Confirm no netdata process running on monitoring-lv3
ssh monitoring-lv3 "pgrep netdata || echo 'Netdata not running (expected)'"

# Confirm Prometheus no longer has netdata scrape targets
curl -s http://monitoring-lv3:9090/api/v1/targets | python3 -m json.tool | grep -c netdata || echo '0 (expected)'

# Confirm realtime.lv3.org returns 404 or is deprovisioned
curl -o /dev/null -s -w "%{http_code}" https://realtime.lv3.org
```
...
- Removes `realtime.lv3.org` nginx edge publication
