...
- OpenBao runs as a two-node Raft HA pair: the primary node on `docker-runtime-lv3` (10.10.10.20) and a peer on `runtime-control-lv3` (10.10.10.92). Raft replicates all secret data between nodes; each node must still be unsealed independently because Shamir manages the per-node encryption key, not the Raft data layer.
...
- OpenBao is live on both nodes with managed initialization and unseal, scoped `userpass` and `AppRole` identities, seeded controller and mail secrets, Transit verification, verified PostgreSQL dynamic credential issuance against `postgres-lv3`, and a `step-ca`-issued external mTLS listener.
