...
- OpenBao is live on `docker-runtime-lv3` with managed initialization and unseal, scoped `userpass` and `AppRole` identities, seeded controller and mail secrets, Transit verification, verified PostgreSQL dynamic credential issuance against `postgres-lv3`, and a `step-ca`-issued external mTLS listener on `https://100.118.189.95:8200`.
