# ADR 0022: Uptime Kuma On The Docker Runtime VM
...
The platform already has internal metrics and dashboards through ADR 0011, but it still lacks a simple service-level uptime view and a low-friction way to add endpoint checks as operational work continues.
...
That makes the remaining gap clear:
...
We will run Uptime Kuma on the Docker runtime VM under `/opt/uptime-kuma`.
...
- repo automation uses that stored local auth plus the officially supported internal Socket.IO management flow to add and update monitors
