...
- `curl -fsS https://n8n.lv3.org/healthz` returned a healthy response through the shared edge
- `curl -fsS http://127.0.0.1:5678/healthz/readiness` succeeded on `docker-runtime-lv3`
- `POST http://127.0.0.1:5678/rest/login` succeeded with the repo-managed owner account
- `POST /rest/owner/setup` was idempotent after the first bootstrap
- the runtime connected successfully to the repo-managed PostgreSQL backend
