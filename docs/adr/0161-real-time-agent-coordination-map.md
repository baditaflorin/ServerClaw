...
The first repository implementation lands in [`platform/coordination/map.py`](/Users/live/Documents/GITHUB_PROJECTS/proxmox_florin_server/platform/coordination/map.py) as a worker-shared file-backed coordination map with heartbeat TTL pruning. This keeps the live state directly readable from the mounted worker checkout while preserving the same per-session contract the future JetStream-backed variant can expose.
