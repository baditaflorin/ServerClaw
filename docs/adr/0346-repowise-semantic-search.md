...
Third-party embedding APIs (Voyage AI, OpenAI, Anthropic) introduce external dependencies and potential data exposure. The platform already runs Ollama and Qdrant on `docker-runtime-lv3`.
...
- **Ansible role** `repowise_runtime`: deploys on `docker-runtime-lv3`, joins `platform-context_default` Docker network to reach existing Qdrant, schedules nightly index rebuild via cron.

No third-party APIs are used.

## Consequences

- Semantic search available at `http://docker-runtime-lv3:7070/search` (internal only until nginx edge is published).
