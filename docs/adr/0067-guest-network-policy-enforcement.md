...
- a compromised container on `docker-runtime` can freely connect to `postgres`, `backup`, or any internal service on any port
...
2. **explicit allow rules** are declared per VM in `inventory/host_vars/proxmox-host.yml` under a new `network_policy:` key, mirroring the existing `guests:` structure
...
|---|---|---|---|
| any guest | monitoring | 8086 (InfluxDB) | telemetry |
| docker-runtime | postgres | 5432 | application DB |
| docker-runtime | step-ca (internal) | 9000 | cert renewal |
| nginx-edge | docker-runtime | service ports | reverse proxy |
| any guest | host | 22 (via step-ca cert) | SSH management |
| host | any guest | 22 | Ansible convergence |

All other flows are blocked by default and must be added via a reviewed change to `proxmox-host.yml`.
...
- Inter-container traffic inside `docker-runtime` is out of scope for this ADR; container network policy is a follow-up concern.
