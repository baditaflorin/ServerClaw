...
- The rebased `main` release `0.175.1` advanced the live platform version to `0.130.22` on `2026-03-26`. The follow-up verification from the merged mainline state also fixed the Windmill sandbox write path by moving durable reaper-run summaries under `/srv/proxmox_florin_server/.local/fixtures/reaper-runs/`, confirmed `f/lv3/ephemeral_vm_reaper` at hash `a2eed27d62fa68fc`, and recorded the clean manual sweep receipt `.local/fixtures/reaper-runs/reaper-run-20260326T170554Z.json`.
