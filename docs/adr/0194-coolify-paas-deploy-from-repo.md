...
- Implemented In Platform Version: 0.130.31 (workstream live apply; canonical `versions/stack.yaml` update remains for merge-to-main)
...
- `uv run --with pytest --with pyyaml --with jsonschema python -m pytest tests/test_coolify_tool.py tests/test_coolify_runtime_role.py tests/test_coolify_playbook.py tests/test_lv3_cli.py -q` passed with `53 passed in 1.01s` on 2026-03-28.
- `python3 -m py_compile scripts/coolify_tool.py` passed on 2026-03-28.
- `./scripts/validate_repo.sh agent-standards` passed on 2026-03-28 after repairing the branch-registry and zero-match arithmetic handling in `scripts/validate_repo.sh`.
- `uv run --with pyyaml --with jsonschema python scripts/validate_repository_data_models.py --validate` remains intentionally blocked on this workstream branch because `versions/stack.yaml` is protected and still reflects `main` rather than the branch-local Coolify live state.

## Live Apply Note

Live apply completed from `codex/ws-0194-live-apply` on 2026-03-28.

- `make converge-coolify` completed successfully and converged `proxmox_florin`, `coolify-lv3`, and `nginx-lv3`.
...
- `python3 scripts/coolify_tool.py deploy-repo --repo https://github.com/coollabsio/coolify-examples --branch main --base-directory /static --app-name repo-smoke --build-pack static --subdomain repo-smoke --wait --timeout 900` finished successfully with deployment `a13akq7v9de7vyqo5u33pz0y`.
- Direct public-edge verification against `65.108.75.123` showed `https://coolify.lv3.org` returning the expected oauth2-proxy `302` challenge, `https://repo-smoke.apps.lv3.org` returning `200`, and `https://apps.lv3.org` returning `404` while no default apex app is assigned.
- `dig +short` from the controller still returned no public answers for `coolify.lv3.org`, `apps.lv3.org`, or `repo-smoke.apps.lv3.org` at verification time, so the live receipt records the successful `--resolve` checks separately from DNS propagation.
