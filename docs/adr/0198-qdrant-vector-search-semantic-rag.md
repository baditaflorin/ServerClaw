...
- Keep Qdrant on `docker-runtime` as the vector store backing platform retrieval.
...
- Semantic retrieval now depends on the Ollama embedding model being present and healthy on `docker-runtime`.
...
- Production live apply now preserves the explicit full `scripts/index_platform_knowledge.py` rebuild as a separate maintenance action because rebuilding the expanded mirrored corpus inline exceeds the safe synchronous Ollama budget on `docker-runtime`.
