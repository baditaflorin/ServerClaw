...
- applied ADR 0043 live from `main` by converging OpenBao on `docker-runtime`, provisioning its PostgreSQL backend inputs on `postgres`, and seeding scoped controller and mail secrets into the managed secret authority
...
- OpenBao is now live on `docker-runtime` with integrated Raft storage, managed init and unseal, scoped `userpass` and `AppRole` identities, verified Transit operations, and a verified PostgreSQL dynamic credential path to `postgres`
