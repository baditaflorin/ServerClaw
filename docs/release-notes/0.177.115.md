...
## Platform Impact
- ADR 0302 is now live on production: docker-runtime writes encrypted Restic file-level backups into the MinIO-backed restic-config-backup repository and the managed backup plus restore verification paths are now evidenced in-repo.
