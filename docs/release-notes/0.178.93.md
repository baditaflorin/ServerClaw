...
## Summary
- [adr-0396 amendments] Implement all 7 ADR 0396 gap amendments from Netdata postmortem — 9 new CATALOG_REGISTRY entries (subdomain-exposure-registry, certificate-catalog, command-catalog, top_level_key type for service-completeness, yaml_topology_block type for inventory topology); _find_role_name reads role_name from catalog (Amendment 3); _remove_inline_service_markers + _remove_role_inline_markers for # SERVICE: annotations in role defaults/Jinja2 templates (Amendment 4); generate_https_tls_assurance.py regenerated with # BEGIN/END SERVICE block markers (Amendment 5); _validate_modified_files post-purge integrity parser (Amendment 6); validate_catalog_registry + --validate-registry self-validation flag (Amendment 1); projected CPU-only decommissioning raised to ~95%
