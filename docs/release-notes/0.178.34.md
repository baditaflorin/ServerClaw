...
## Summary
- feat(ADR-0376): identity-core watchdog — 30s OIDC discovery probe on Keycloak, auto-restart on 2 failures (6/hr limit), oauth2-proxy watchdog on nginx-edge, KC_CACHE=local to kill JGroups TCP loop
...
- Fixed browser-runner service accessibility — added firewall rules to open port 8096 on docker-runtime from management network, all guests, and Docker bridge networks; service now accessible from headscale at http://10.10.10.20:8096/
...
- fix Woodpecker OAuth login: add nginx edge + TLS for git.example.com, fix WOODPECKER_GITEA_URL to use private IP (cross-host LAN), remove dead gitea_runtime_default external network from Woodpecker compose
...
- ADR 0375: Certificate validation and concordance enforcement — automated SSL checking for all edge-published domains, pre-push gate integration, Uptime Kuma monitors, and fix playbook for NGINX edge certificate hostname mismatches (ci.example.com, bi.example.com, grist.example.com, paperless.example.com, annotate.example.com, ntfy.example.com); certificate_validator.py now uses certificate-catalog.json (ADR 0101 canonical) as primary source; pre-push gate ADR ref corrected to 0375; validate-certificates Makefile target hooked into configure-edge-publication; back-references added to ADR 0101 and ADR 0273.
