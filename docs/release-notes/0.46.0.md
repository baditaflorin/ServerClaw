...
- applied ADR 0042 live from `main` by converging the private `step-ca` runtime on `docker-runtime`, publishing it through the Proxmox host Tailscale address, and installing CA-backed SSH host trust on the Proxmox host and managed guests
...
- `step-ca` is now live on `docker-runtime` and reachable privately at `https://100.118.189.95:9443`
