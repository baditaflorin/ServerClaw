...
- applied ADR 0057 live by provisioning the Mattermost PostgreSQL backend on `postgres`, converging the private Mattermost runtime on `docker-runtime`, and publishing operator access through the Proxmox host Tailscale proxy on port `8066`
...
- Mattermost is now live on `docker-runtime` and reachable privately at `http://100.118.189.95:8066`
...
- Grafana on `monitoring` now carries the `lv3-mattermost-platform-alerts` contact point for repo-managed alert routing into Mattermost
