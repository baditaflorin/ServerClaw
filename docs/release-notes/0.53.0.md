...
- implemented ADR 0059 live by installing `ntopng` on `proxmox-host`, capturing the private guest bridge directly from `vmbr10`, and adding edge-adjacent context from `vmbr0`
...
- ntopng is now running on `proxmox-host` and is reachable privately at `http://100.118.189.95:3001`
