...
- applied ADR 0055 live from `main` by converging Portainer on `docker-runtime`, publishing it privately through the Proxmox host Tailscale proxy on port `9444`, and persisting controller-local bootstrap auth under `.local/portainer`
...
- Portainer CE is now live on `docker-runtime` and reachable privately at `https://100.118.189.95:9444`
