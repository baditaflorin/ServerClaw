...
- Updated the governed subdomain catalog to require `edge_oidc` for `docs.example.com`, `changelog.example.com`, and `ops.example.com`, while preserving Grafana's `upstream_auth` login boundary.
...
- live platform version advances to `0.114.7` because ADR 0133 was applied and verified on nginx-edge on 2026-03-24

## Residual Risk
- `status.example.com`, `uptime.example.com`, and other intentionally public informational surfaces still rely on correct catalog classification; future portal additions must not bypass that review path.
