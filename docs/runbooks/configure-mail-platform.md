...
- public DNS for `mail.example.com` and the `example.com` MX record
...
1. `server@example.com` is the desired managed mailbox and the three default notification profiles should exist:
   - `alerts@example.com`
   - `platform@example.com`
   - `agents@example.com`
2. the public IP `203.0.113.1` can receive SMTP traffic on TCP `25`, `587`, and `993`
3. `HETZNER_DNS_API_TOKEN` is set on the controller
4. `/Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/mail-platform/brevo-api-key.txt` exists and contains the Brevo transactional API key
...
- `example.com` MX record pointing at `mail.example.com`
- SPF and DMARC TXT records for `example.com`
...
- Grafana dashboard `lv3-mail-platform` on `monitoring`
...
- `/Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/mail-platform/stalwart-admin-password.txt`
- `/Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/mail-platform/server-mailbox-password.txt`
- `/Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/mail-platform/metrics-password.txt`
- `/Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/mail-platform/gateway-api-key.txt`
- `/Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/mail-platform/profiles/operator-alerts-mailbox-password.txt`
- `/Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/mail-platform/profiles/operator-alerts-gateway-api-key.txt`
- `/Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/mail-platform/profiles/platform-transactional-mailbox-password.txt`
- `/Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/mail-platform/profiles/platform-transactional-gateway-api-key.txt`
- `/Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/mail-platform/profiles/agent-reports-mailbox-password.txt`
- `/Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/mail-platform/profiles/agent-reports-gateway-api-key.txt`
...
- admin header: `X-API-Key: $(cat /Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/mail-platform/gateway-api-key.txt)`
- scoped send header example: `X-API-Key: $(cat /Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/mail-platform/profiles/platform-transactional-gateway-api-key.txt)`
...
1. `operator-alerts` -> `alerts@example.com`
2. `platform-transactional` -> `platform@example.com`
3. `agent-reports` -> `agents@example.com`
...
```sh
curl -s \
  -H "X-API-Key: $(cat /Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/mail-platform/profiles/platform-transactional-gateway-api-key.txt)" \
  -H "Content-Type: application/json" \
  -d '{"to":["operator@example.com"],"subject":"LV3 mail test","text":"mail platform is live"}' \
  http://10.10.10.92:8081/send
```
...
```sh
curl -s \
  -H "X-API-Key: $(cat /Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/mail-platform/gateway-api-key.txt)" \
  http://10.10.10.92:8081/v1/profiles
```
...
curl -s \
  -H "X-API-Key: $(cat /Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/mail-platform/gateway-api-key.txt)" \
  http://10.10.10.92:8081/v1/domains
```
...
curl -s \
  ...
  -H "X-API-Key: $(cat /Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/mail-platform/gateway-api-key.txt)" \
  -H "Content-Type: application/json" \
  -d '{"emails":["server@example.com"]}' \
  http://10.10.10.92:8081/v1/mailboxes/server
```
...
2. `ssh -i /Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/ssh/hetzner_llm_agents_ed25519 -o IdentitiesOnly=yes -J ops@100.64.0.1 ops@10.10.10.92 'docker compose --file /opt/mail-platform/docker-compose.yml ps && sudo ls -l /opt/mail-platform/openbao /run/lv3-secrets/mail-platform && sudo test ! -e /opt/mail-platform/gateway/gateway.env'`
3. `ssh -i /Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/ssh/hetzner_llm_agents_ed25519 -o IdentitiesOnly=yes -J ops@100.64.0.1 ops@10.10.10.92 'python3 /usr/local/libexec/lv3-mail-platform-metrics.py'`
4. `curl -s -H "X-API-Key: $(cat /Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/mail-platform/gateway-api-key.txt)" http://10.10.10.92:8081/v1/profiles`
5. `curl -s -H "X-API-Key: $(cat /Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/mail-platform/gateway-api-key.txt)" http://10.10.10.92:8081/v1/mailboxes`
6. `ansible-playbook -i /Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/inventory/hosts.yml /Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/playbooks/mail-platform-notification-profiles-verify.yml --private-key /Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/ssh/hetzner_llm_agents_ed25519 -e proxmox_guest_ssh_connection_mode=proxmox_host_jump --limit runtime-control`
7. `curl -I https://grafana.example.com`
8. `ssh -i /Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/ssh/hetzner_llm_agents_ed25519 -o IdentitiesOnly=yes -J ops@100.64.0.1 ops@10.10.10.40 'curl -fsS http://127.0.0.1:3200/api/search/tag/service.name/values | jq -r ''.tagValues[]'' | grep mail-gateway'`
9. `ssh -i /Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/.local/ssh/hetzner_llm_agents_ed25519 -o IdentitiesOnly=yes -J ops@100.64.0.1 ops@10.10.10.92 'python3 - <<'"'"'PY'"'"'\nimport smtplib\nfrom pathlib import Path\npassword = Path(\"/etc/lv3/mail-platform/server-mailbox-password\").read_text().strip()\nclient = smtplib.SMTP(\"10.10.10.92\", 1587, timeout=10)\nclient.ehlo()\nassert \"starttls\" not in client.esmtp_features, client.esmtp_features\nprint(client.login(\"server\", password))\nclient.quit()\nPY'`
