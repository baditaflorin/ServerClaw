...
the private `draw.lv3.org` surface.

## Result

- `docker-runtime-lv3` runs the Excalidraw frontend and the collaboration room from `/opt/excalidraw`
- `draw.lv3.org` is published through the shared NGINX edge behind the repo-managed oauth2-proxy gate
...
Verify the private Excalidraw frontend listener on `docker-runtime-lv3`:
...
Verify public DNS is present for `draw.lv3.org`:

```bash
dig +short draw.lv3.org @8.8.8.8
```

Verify the public hostname is routed through the shared authenticated edge:

```bash
curl -Ik --resolve draw.lv3.org:443:65.108.75.123 https://draw.lv3.org
```
...
- Keep `draw.lv3.org` inside the shared authenticated-edge set so the architecture diagrams stay private to operators.
