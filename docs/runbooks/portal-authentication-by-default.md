...
uv run --with pyyaml --with jsonschema python -m unittest \
  ...
  tests.test_authentik_runtime_role \
...
uv run --with playwright python scripts/session_logout_verify.py \
  --password-file .local/authentik/bootstrap-password.txt
```
...
- the verifier proves the final post-logout challenge on both `home.example.com` and `wiki.example.com`
...
If the Authentik authority or app-local logout consumers changed, converge them before rechecking the portals:
