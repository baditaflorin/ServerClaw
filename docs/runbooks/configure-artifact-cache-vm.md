...
`artifact-cache-lv3` as VM `180`, converge the private cache-plane runtime on
that guest, and then repoint the build host to consume the new cache plane.

## Model Summary

- create `artifact-cache-lv3` as VM `180` on `10.10.10.80`
...
- move build and CI consumers first by rewiring `docker-build-lv3`
...
1. Ensures the Proxmox host has the `artifact-cache-lv3` guest defined as VM `180`.
...
4. Waits for SSH on `artifact-cache-lv3`.
5. Converges the guest firewall, named `ops` access, Docker runtime, and artifact-cache containers.
6. Configures Docker on `artifact-cache-lv3` to trust its own internal
...
8. Replays the governed `build-artifact-cache` service so `docker-build-lv3` uses the new cache host and drops the old local cache stack.
...
```bash
ssh -i /Users/live/Documents/GITHUB_PROJECTS/proxmox_florin_server/.local/ssh/hetzner_llm_agents_ed25519 -o IdentitiesOnly=yes ops@100.64.0.1 'sudo qm list | grep 180'
```
...
- Do not publish `artifact-cache-lv3` through the public edge.
