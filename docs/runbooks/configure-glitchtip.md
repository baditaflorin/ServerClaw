...
```bash
curl -fsS https://errors.example.com/api/0/internal/health/
curl -fsS https://errors.example.com/_allauth/browser/v1/config | grep -E 'LV3 Keycloak|sso\.lv3\.org|openid-configuration'
...
The repo-managed publication verification now waits for a quiet controller window across `docker-runtime` and `nginx-edge` before probing `errors.example.com`, and it retries once after that quiet window if another live apply had just been mutating the shared runtime or edge.
- The exact-main recovery path now repairs Docker bridge-chain publication on
  `docker-runtime` before the GlitchTip public verifier runs, so reruns
  can recover from shared-runtime nftables drift without a manual guest-side
  bridge-chain repair.

## Notes

- GlitchTip owns browser authentication itself through the repo-managed Keycloak OIDC client, so `errors.example.com` is published with `upstream_auth` instead of the shared edge oauth2-proxy boundary
