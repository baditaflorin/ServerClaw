# Runbook: retired-deployment Full-Day Deployment (ADR 0431)
...
3. **Four overlay files** in `.local/` on the control workstation:
   ...
   | File | Purpose |
   |------|---------|
   | `.local/identity.yml.retired-deployment` | Scalar overlay (domain, IPs, operator email) — see ADR 0424 §4 |
...
```bash
make preflight-retired-deployment
```
...
```bash
make deploy-retired-deployment
```

This runs `playbooks/retired-deployment-full-day.yml`, which imports in order:
...
partial failures can be resumed with `make converge-retired-deployment-chain`.
...
```bash
make smoke-retired-deployment-mail
```
...
| Situation | Target |
|-----------|--------|
| Overlay-only edit (new VM, changed IP) | `make converge-retired-deployment-chain` |
| Mail-platform redeploy only | `make converge-mail-platform env=clone` |
| Just re-send the acceptance email | `make smoke-retired-deployment-mail` |
...
mid-run failure requires re-invoking either the full `deploy-retired-deployment` or
the narrower `converge-retired-deployment-chain` to resume.
...
|---------|--------------|---------------|
| `MISSING: .local/identity.yml.retired-deployment` | Overlay never created | ADR 0424 §4 for the full scalar list |
...
| `proxmox-install` fails at `proxmox_network` | Hetzner bridge gateway mismatch | Verify `management_gateway4` in `.local/identity.yml.retired-deployment` matches `ip route` on the host |
