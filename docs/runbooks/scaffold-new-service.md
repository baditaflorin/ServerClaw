...
The ADR 0107 formal operator workflow now lives in [add-a-new-service.md](add-a-new-service.md). Use this runbook as the lower-level reference for the generator itself.
...
- refreshed generated service aggregate entries in:
  - `inventory/host_vars/<topology-host>.yml`
...
4. Add `degradation_modes` in `catalog/services/<service-id>/service.yaml` if the service can stay reachable while a declared soft dependency is impaired.
5. Review `inventory/host_vars/<topology-host>.yml` and adjust DNS or edge publication details.
