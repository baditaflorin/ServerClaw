...
repo-managed jobs with Authentik OIDC for routine operator sign-in and a
...
- Authentik client manifest: [config/authentik/oauth-clients.yaml](../../config/authentik/oauth-clients.yaml)
...
- controller-local Authentik client secret: `.local/authentik/semaphore-client-secret.txt`

## Access Model

- Semaphore is published only through the private controller URL exposed by the
  Proxmox host Tailscale proxy.
- `playbooks/semaphore.yml` consumes the dedicated Authentik `semaphore`
  client before the runtime converge and mirrors its secret under
  `.local/authentik/semaphore-client-secret.txt`.
...
- let `playbooks/semaphore.yml` own the dedicated Authentik client; do not
...
- if the Semaphore Authentik client secret is rotated or exposed, refresh the
...
### Authentik readiness probe fails during converge

If `make converge-semaphore env=production` fails while waiting for the
Authentik readiness endpoint, follow the governed Authentik recovery path:

```bash
make live-apply-service service=authentik env=production ALLOW_IN_PLACE_MUTATION=true
curl -fsS https://id.example.com/-/health/ready/
```
