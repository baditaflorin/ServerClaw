...
## Authentik OIDC access and browser verification

Gitea's Authentik OAuth source requires the `gitea-users` group. That group is
login-only: the existing platform-admin group remains the sole mapping to the
`ops/Owners` team, and membership in `gitea-users` does not grant Gitea admin
rights or repository ownership. Add ordinary operators to `gitea-users` in
Authentik when they should be able to sign in to Gitea.

The normal `config/authentik/identities.yaml` reconciliation manages the stable
...
root. It starts with a fresh browser profile, validates TLS using the local CA,
completes the Authentik login, returns to Gitea, and asserts that the session is
`gitea-e2e` with `is_admin: false`:
...
The check reads the password from the private local file and deliberately
redacts browser/OIDC diagnostics that could contain a state token or code.
It waits for the exact signed-in username in Gitea's visible user menu and
checks the public profile's `is_admin` field; Gitea does not authenticate the
`/api/v1/user` endpoint using browser-cookie sessions. Keep the `gitea-e2e`
account non-admin; remove it and its local password file when the verification
