...
at `https://bi.lv3.org`. The runtime lives on `docker-runtime-lv3`, stores its
metadata in the `superset` schema of the shared `postgres` database on
`postgres-lv3`, and delegates routine browser sign-in to Keycloak.
...
- `HETZNER_DNS_API_TOKEN` in the environment so the playbook can publish
  `bi.lv3.org`
- functional reachability from `docker-runtime-lv3` to `postgres-lv3` and the
  shared Keycloak realm

The playbook performs these steps:

1. Ensures the `bi.lv3.org` Hetzner DNS record exists.
2. Creates or reconciles the `superset` metadata schema, the `superset`
   metadata role, and the `superset_reader` datasource role on `postgres-lv3`.
3. Mirrors the generated database credentials and datasource catalog to the
   ...
   secret mirror on `docker-runtime-lv3`.
...
python3 scripts/superset_bootstrap.py verify-public \
  --base-url https://bi.lv3.org \
  --expected-postgres-databases-file /Users/live/Documents/GITHUB_PROJECTS/proxmox_florin_server/.local/superset/postgres-databases.json \
  ...
  --admin-password-file /Users/live/Documents/GITHUB_PROJECTS/proxmox_florin_server/.local/superset/admin-password.txt \
  --expected-extra-database "Plausible ClickHouse" \
  --report-file /Users/live/Documents/GITHUB_PROJECTS/proxmox_florin_server/.local/superset/verify-public-report.json
```

The verification helper checks:

- `GET /health`
- `GET /login/keycloak` redirects to `sso.lv3.org`
...
- If the runtime fails to publish after a Docker bridge-chain loss on
  `docker-runtime-lv3`, rerun `make converge-superset`. The role includes the
...
- If the managed datasource inventory drifts from the current PostgreSQL
  ...
  from `postgres-lv3` before the managed Superset metadata objects are
  reconciled.
