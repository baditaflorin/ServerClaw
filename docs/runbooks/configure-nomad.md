...
- Nomad clients on `docker-runtime-lv3`, `runtime-general-lv3`, `runtime-ai-lv3`, and `docker-build-lv3`
- a Proxmox-host Tailscale TCP proxy for private controller access
- dedicated `runtime-general` and `runtime-ai` namespaces for the first pool-scoped scheduling boundaries
...
2. `monitoring-lv3`, `docker-runtime-lv3`, `runtime-general-lv3`, `runtime-ai-lv3`, and `docker-build-lv3` are reachable through the Proxmox jump path
...
- Nomad clients on `docker-runtime-lv3`, `runtime-general-lv3`, `runtime-ai-lv3`, and `docker-build-lv3` with the Docker driver enabled
- dedicated `runtime-general` and `runtime-ai` namespaces for pool-scoped workloads
...
- The `runtime-general` and `runtime-ai` namespaces are the first pool-scoped scheduler boundaries introduced by ADR 0319 and ADR 0320. Keep support surfaces and AI-bursty jobs there instead of reusing the broad `default` namespace.
