...
- the local Ollama connector is enabled from repo-managed vars and points to `http://host.docker.internal:11434`
...
- `.local/open-webui/provider.env`
  - include only approved external connector variables such as `OPENAI_API_KEY` and `OPENAI_API_BASE_URL`
  - keep `ENABLE_OLLAMA_API`, `OLLAMA_BASE_URL`, `DEFAULT_MODELS`, and `DEFAULT_PINNED_MODELS` repo-managed unless the ADR owner explicitly changes the local-model policy
...
- Do not enable arbitrary outbound connectors in the UI. Add approved external-provider settings through `.local/open-webui/provider.env` and rerun the workflow.
