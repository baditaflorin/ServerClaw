...
The machine-readable source of truth is [config/health-probe-catalog.json](/Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/config/health-probe-catalog.json). Role-owned services must also import `tasks/verify.yml` from their `tasks/main.yml`.
...
- `uptime_kuma`: whether the service is mirrored into the generated [config/uptime-kuma/monitors.json](/Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/config/uptime-kuma/monitors.json)

Degraded-state rules remain service-level metadata in [config/service-capability-catalog.json](/Users/live/Documents/GITHUB_PROJECTS/proxmox-host_server/config/service-capability-catalog.json) under `degradation_modes`. If a service has no declared degraded modes, broken readiness is treated as a failure rather than an acceptable partial state.
...
| --- | --- | --- | --- | --- | --- |
| `nginx_edge` | `nginx-edge` | implicit from liveness/readiness | `systemctl is-active nginx` | HTTPS `GET /` with `Host: nginx.example.com` | yes |
| `grafana` | `monitoring` | implicit from liveness/readiness | `GET http://127.0.0.1:3000/api/health` | LV3 dashboard and datasource API lookups | yes |
| `proxmox_ui` | `proxmox-host` | implicit from liveness/readiness | TCP `10.10.10.1:8006` | `GET /api2/json/version` | yes |
| `docker_runtime` | `docker-runtime` | implicit from liveness/readiness | `systemctl is-active docker` | `docker info` | no |
| `docker_build` | `docker-build` | implicit from liveness/readiness | TCP `10.10.10.30:22` | SSH reachability | yes |
| `uptime_kuma` | `docker-runtime` | implicit from liveness/readiness | running `uptime-kuma` container | local HTTP UI returns 200 | yes |
| `mail_platform` | `docker-runtime` | implicit from liveness/readiness | `GET /healthz` on the private gateway | authenticated gateway plus Stalwart catalog checks | yes |
| `step_ca` | `docker-runtime` | implicit from liveness/readiness | TCP `127.0.0.1:9000` | issue and verify a short-lived test certificate | yes |
| `windmill` | `docker-runtime` | implicit from liveness/readiness | `GET /api/version` | seeded `windmill_healthcheck` job returns the expected payload | yes |
| `netbox` | `docker-runtime` | API token bootstrap path | `GET /login/` | bootstrap API token can read `/api/users/users/` | yes |
| `open_webui` | `docker-runtime` | admin sign-in succeeds locally | `GET /` | repo-managed admin sign-in succeeds | yes |
| `openbao` | `docker-runtime` | active-node health endpoint | `GET /v1/sys/seal-status` | `GET /v1/sys/health` returns active node status | no |
| `ntopng` | `proxmox-host` | implicit from liveness/readiness | local HTTP root returns `200` or `302` | authenticated interface inventory and traffic counters return data | no |
| `portainer` | `docker-runtime` | implicit from liveness/readiness | TCP `127.0.0.1:9443` | Portainer API exposes the managed local endpoint | yes |
| `postgres` | `postgres` | implicit from liveness/readiness | TCP `127.0.0.1:5432` | `psql -Atqc 'SELECT 1'` | yes |
| `backup_pbs` | `backup` | implicit from liveness/readiness | TCP `127.0.0.1:8007` | `proxmox-backup-manager datastore list` includes the managed datastore | yes |
