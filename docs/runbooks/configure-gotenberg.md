...
- `runtime-ai` runs the stateless Gotenberg runtime from `/opt/gotenberg`
- the private runtime listens on `10.10.10.90:3007` for guest-network callers
- the shared API gateway targets `/v1/gotenberg` on `https://api.example.com` for authenticated operator and automation access
...
Verify the local health endpoint on `runtime-ai`:
...
curl -fsS -o "$pdf" \
  ...
  https://api.example.com/v1/gotenberg/forms/chromium/convert/html
head -c 4 "$pdf"
```
