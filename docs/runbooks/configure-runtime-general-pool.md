...
split by provisioning `runtime-general`, enrolling it as a Nomad client,
standing up the Traefik plus Dapr substrate, and moving the first lightweight
operator and support services off `docker-runtime`.

## Result

- Proxmox guest `runtime-general` exists at `10.10.10.91` with VMID `191`
- `runtime-general` runs Docker, the repo-managed guest firewall, Traefik on `9080`, and Dapr on `3500`
- `monitoring` exposes the `runtime-general` Nomad namespace and sees `runtime-general` as a client
- Uptime Kuma, the public status-page backing route, Homepage, and Mailpit run on `runtime-general`
- the legacy Uptime Kuma, Homepage, and Mailpit copies are stopped on `docker-runtime`
- the shared NGINX edge continues to publish `uptime.example.com`, `status.example.com`, and `home.example.com` against the new runtime-general upstreams
...
ANSIBLE_HOST_KEY_CHECKING=False ansible -i inventory/hosts.yml monitoring -m shell \
  -a 'sudo /usr/local/bin/lv3-nomad namespace status runtime-general && sudo /usr/local/bin/lv3-nomad node status | grep -F runtime-general' \
...
```bash
curl -Ik https://uptime.example.com
curl -Ik https://status.example.com
curl -Ik https://home.example.com
```

Verify the legacy support-service copies are no longer running on `docker-runtime`:
...
- The current Proxmox host only exposes the base template VM `9000`, so `runtime-general` clones from `lv3-debian-base` and the playbook's `docker_runtime` role installs Docker during first converge.
- The pool live apply replays `lv3.platform.proxmox_network` on the host after provisioning `runtime-general`. Keep that host-side step in place because the Nomad scheduler and other existing guests need their Proxmox VM firewall files refreshed whenever a new pool guest appears.
- The dedicated `runtime-general` substrate replay now keeps an already
...
- The pool live apply also replays `lv3.platform.linux_guest_firewall` on `monitoring` after the new guest appears. Keep that targeted guest-side replay in place so the Nomad scheduler admits `runtime-general`, but do not broaden it back to the entire guest set unless the shared-runtime Docker bridge behavior is hardened first.
- The first successful runtime-general service replay now restores the legacy `docker-runtime` Uptime Kuma data directory onto `runtime-general` before retirement and records `/opt/uptime-kuma/.legacy-data-restored` as the one-time migration marker. If that marker is absent unexpectedly, inspect both guests before forcing another replay.
- The legacy-retirement phase now fails closed unless the same playbook run has already verified the `runtime-general` routes and the shared edge publication. Do not bypass that guard with `--start-at-task`, `--limit docker-runtime`, or ad hoc retirement-only replays.
- Homepage now binds both the guest address and loopback on `runtime-general` so the shared Traefik router can keep using `127.0.0.1:3090` without exposing a second public edge path. If `/homepage` returns `502`, confirm the loopback listener first.
- Traefik and Dapr on `runtime-general` are private infrastructure surfaces. Do not publish them directly on the public edge.
- `runtime-general` is the supported place for lightweight operator and support surfaces such as Uptime Kuma, Homepage, and Mailpit. Do not reintroduce these workloads on `docker-runtime` without a new ADR or rollback decision.
