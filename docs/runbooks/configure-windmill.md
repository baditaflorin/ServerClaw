...
- Repo-managed Windmill raw apps should commit a `package-lock.json` whenever they ship a frontend `package.json`. `make converge-windmill` now resolves those dependencies inside the sync container before `wmill sync push`, using `npm ci --no-audit --no-fund` when the lockfile is present and only falling back to `npm install --no-package-lock --no-audit --no-fund` when no lockfile exists.
