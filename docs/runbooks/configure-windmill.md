...
- ADR 0316 adds repo-managed worker-side state under `.local/state/journey-analytics/` plus the seeded helpers `f/lv3/operator_journey_event` and `f/lv3/operator_journey_scorecards`. Keep the worker checkout writable there and keep the mirrored Glitchtip findings event URL present before expecting bounded failure milestones to emit successfully.
