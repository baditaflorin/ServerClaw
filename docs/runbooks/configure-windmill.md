...
5. `WINDMILL_TOKEN="$(cat /Users/live/Documents/GITHUB_PROJECTS/proxmox_florin_server/.local/windmill/superadmin-secret.txt)" python3 scripts/windmill_run_wait_result.py --base-url http://100.64.0.1:8005 --workspace lv3 --path f/lv3/windmill_healthcheck --payload-json '{"probe":"manual-run"}'`
...
10. `WINDMILL_TOKEN="$(cat /Users/live/Documents/GITHUB_PROJECTS/proxmox_florin_server/.local/windmill/superadmin-secret.txt)" python3 scripts/windmill_run_wait_result.py --base-url http://100.64.0.1:8005 --workspace lv3 --path f/lv3/config_merge/merge_config_changes --payload-json '{}'`
...
12. `WINDMILL_TOKEN="$(cat /Users/live/Documents/GITHUB_PROJECTS/proxmox_florin_server/.local/windmill/superadmin-secret.txt)" python3 scripts/windmill_run_wait_result.py --base-url http://100.64.0.1:8005 --workspace lv3 --path f/lv3/ephemeral_vm_reaper --payload-json '{}'`
...
- For ad hoc script verification, use `scripts/windmill_run_wait_result.py` instead of calling `/api/w/<workspace>/jobs/run_wait_result/p/...` directly. The helper submits by path, falls back to a script-hash run when Windmill rejects the direct path contract, and then polls the job until it reaches a terminal state.
...
- Repo-managed Windmill raw apps now install frontend dependencies inside the guest-side seed staging directory before `wmill sync push`. Keep each raw app `package-lock.json` current when frontend dependencies change so converges can use deterministic `npm ci`.
