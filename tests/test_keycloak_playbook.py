...
def test_keycloak_playbook_targets_the_docker_runtime_vm() -> None:
    playbook = yaml.safe_load(PLAYBOOK_PATH.read_text())

    runtime_play = next(play for play in playbook if play["name"] == "Converge Keycloak on the Docker runtime VM")
    ...
    assert grafana_role["vars"]["grafana_sso_client_secret"] == (
        "{{ hostvars[playbook_execution_host_patterns.docker_runtime[playbook_execution_env]].keycloak_grafana_client_secret }}"
    )


def test_keycloak_service_topology_points_at_docker_runtime() -> None:
    ...
    for topology in (group_topology, public_edge_topology):
        assert topology["owning_vm"] == "docker-runtime-lv3"
        assert topology["private_ip"] == "10.10.10.20"
        assert topology["edge"]["upstream"] == "http://10.10.10.20:8091"
        assert topology["urls"]["internal"] == "http://10.10.10.20:8091"

    assert host_topology["owning_vm"] == "docker-runtime-lv3"
    assert "docker-runtime-lv3" in host_topology["private_ip"]
    assert "docker-runtime-lv3" in host_topology["edge"]["upstream"]
    assert "docker-runtime-lv3" in host_topology["urls"]["internal"]
