...
def test_keycloak_service_topology_points_at_runtime_control() -> None:
    ...
    for topology in (group_topology, public_edge_topology):
        assert topology["owning_vm"] == "runtime-control-lv3"
        assert topology["private_ip"] == "10.10.10.92"
        assert topology["edge"]["upstream"] == "http://10.10.10.92:8091"
        assert topology["urls"]["public"] == "https://sso.lv3.org"

    assert host_topology["owning_vm"] == "runtime-control-lv3"
    assert "runtime-control-lv3" in host_topology["private_ip"]
    assert "runtime-control-lv3" in host_topology["edge"]["upstream"]
    assert "runtime-control-lv3" in host_topology["urls"]["internal"]
