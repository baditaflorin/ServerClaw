...
def test_role_defaults_pin_expected_runtime_paths() -> None:
    ...
    assert defaults["searxng_runtime_public_base_url"] == "{{ platform_service_topology.searxng.urls.public | default(searxng_runtime_controller_url) }}"
...
def test_role_generates_and_mirrors_stable_secret_key() -> None:
    ...
    mirror_task = next(task for task in tasks if task.get("name") == "Mirror the SearXNG secret key to the control machine")
...
def test_verify_tasks_hit_root_and_json_search_endpoints() -> None:
    ...
    search_task = next(task for task in verify_tasks if task.get("name") == "Verify the SearXNG JSON search endpoint responds")
