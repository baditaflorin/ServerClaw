...
def test_defaults_pin_the_runtime_ai_substrate_contract() -> None:
    ...
    assert defaults["runtime_pool_substrate_pool_id"] == "runtime-ai"
    assert defaults["runtime_pool_substrate_entrypoint_name"] == "{{ runtime_pool_substrate_pool_id }}"
    ...
    assert defaults["runtime_pool_substrate_dapr_app_id"] == "{{ runtime_pool_substrate_pool_id }}-router"
    ...
    assert [route["route_id"] for route in defaults["runtime_pool_substrate_routes"]] == [
        "tika",
        "gotenberg",
        "tesseract",
    ]
...
def test_argument_specs_cover_traefik_dapr_and_upstream_inputs() -> None:
    ...
    assert options["runtime_pool_substrate_pool_id"]["type"] == "str"
    assert options["runtime_pool_substrate_entrypoint_name"]["type"] == "str"
    ...
    assert options["runtime_pool_substrate_routes"]["type"] == "list"
...
def test_verify_tasks_cover_traefik_ping_dapr_metadata_and_dapr_invoke() -> None:
    ...
    dapr_invoke_task = next(
        task for task in tasks if task["name"] == "Verify the Dapr sidecar can invoke the Traefik ping route"
    )
    ...
    assert (
        ...
        in dapr_invoke_task["ansible.builtin.command"]["argv"][-1]
    )
...
def test_templates_define_host_networked_traefik_and_dapr_router() -> None:
    ...
    assert "{{ runtime_pool_substrate_traefik_container_name }}:" in compose_template
    assert "{{ runtime_pool_substrate_dapr_container_name }}:" in compose_template
    assert "network_mode: host" in compose_template
    assert "--entrypoints.{{ runtime_pool_substrate_entrypoint_name }}.address" in compose_template
    assert "./daprd" in compose_template
    assert "--app-id" in compose_template
    assert "{% for route in runtime_pool_substrate_routes %}" in router_template
    assert "{{ runtime_pool_substrate_pool_id }}-{{ route.route_id }}" in router_template
