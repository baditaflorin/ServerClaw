...
def test_postgres_role_provisions_named_database_and_role() -> None:
    ...
    assert (
        ...
        == "CREATE DATABASE {{ n8n_database_name }} OWNER {{ n8n_database_user }}"
    )
...
def test_runtime_recovers_missing_docker_bridge_chains_before_startup() -> None:
    ...
    nat_check = next(
        task for task in tasks if task.get("name") == "Check whether the Docker nat chain exists before n8n startup"
    )
    forward_check = next(
        task for task in tasks if task.get("name") == "Check whether the Docker forward chain exists before n8n startup"
    )
    restart = next(
        task
        for task in tasks
        if task.get("name") == "Restart Docker when the nat or forward chain is missing before n8n startup"
    )
    assert_task = next(
        task for task in tasks if task.get("name") == "Assert Docker bridge chains are present before n8n startup"
    )
    startup = next(
        task for task in tasks if task.get("name") == "Start the n8n runtime and recover Docker bridge-chain failures"
    )
    ...
    rescue_fact = next(
        task for task in startup["rescue"] if task.get("name") == "Flag Docker bridge-chain failures during n8n startup"
    )
    retry_task = next(
        task for task in startup["rescue"] if task.get("name") == "Retry n8n startup after Docker bridge-chain recovery"
    )
    recheck_nat = next(task for task in tasks if task.get("name") == "Recheck the Docker nat chain before n8n startup")
    recheck_forward = next(
        task for task in tasks if task.get("name") == "Recheck the Docker forward chain before n8n startup"
    )

    assert start_task["ansible.builtin.command"]["argv"][-3:] == ["up", "-d", "--remove-orphans"]
    assert (
        "No chain/target/match by that name"
        in rescue_fact["ansible.builtin.set_fact"]["n8n_docker_bridge_chain_missing"]
    )
    assert (
