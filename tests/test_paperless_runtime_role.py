...
def test_defaults_define_public_oidc_runtime_and_taxonomy_contract() -> None:
    ...
    assert defaults["paperless_public_hostname_overrides"][0]["hostname"] == "{{ paperless_service_topology.public_hostname }}"
    assert defaults["paperless_public_hostname_overrides"][1]["hostname"] == "{{ hostvars['proxmox_florin'].lv3_service_topology.keycloak.public_hostname }}"
...
def test_templates_enable_public_proxy_headers_and_named_state_volumes() -> None:
    ...
    assert 'PAPERLESS_DBPASS=[[ with secret "kv/data/{{ paperless_openbao_secret_path }}" ]][[ .Data.data.PAPERLESS_DBPASS ]][[ end ]]' in env_ctemplate
    assert 'PAPERLESS_ADMIN_PASSWORD=[[ with secret "kv/data/{{ paperless_openbao_secret_path }}" ]][[ .Data.data.PAPERLESS_ADMIN_PASSWORD ]][[ end ]]' in env_ctemplate
...
def test_inventory_exposes_paperless_to_edge_monitoring_and_private_callers() -> None:
    ...
    monitoring_rule = next(rule for rule in docker_runtime_rules if rule["source"] == "monitoring-lv3" and 8018 in rule["ports"])
