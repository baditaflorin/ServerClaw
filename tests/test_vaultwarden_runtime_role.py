...
def test_defaults_use_private_service_topology_and_local_bootstrap_artifacts() -> None:
    ...
    assert (
        defaults["vaultwarden_admin_token_hash_local_file"]
        == "{{ vaultwarden_local_artifact_dir }}/admin-token-argon2.txt"
    )
    assert (
...
def test_role_generates_hash_with_the_vaultwarden_binary_when_missing() -> None:
    ...
    missing_fact = next(
        task
        for task in tasks
        if task.get("name") == "Record whether the Vaultwarden admin token hash must be generated"
    )
    hash_task = next(
        task
        for task in tasks
        if task.get("name") == "Generate the Vaultwarden admin token hash on the runtime host when missing"
    )
    assert (
        "vaultwarden_admin_token_hash_local.stat.size"
        in missing_fact["ansible.builtin.set_fact"]["vaultwarden_admin_token_hash_missing"]
    )
...
def test_role_recovers_missing_docker_bridge_chains_before_startup() -> None:
    ...
    recreate_fact = next(
        task
        for task in tasks
        if task.get("name") == "Record whether Vaultwarden must be force recreated before startup"
    )
    startup_command = next(task for task in tasks if task.get("name") == "Build the Vaultwarden startup command")
    nat_check = next(
        task
        for task in tasks
        if task.get("name") == "Check whether the Docker nat chain exists before Vaultwarden startup"
    )
    forward_check = next(
        task
        for task in tasks
        if task.get("name") == "Check whether the Docker forward chain exists before Vaultwarden startup"
    )
    restart = next(
        task
        for task in tasks
    ...
    assert_task = next(
        task
        for task in tasks
        if task.get("name") == "Assert Docker bridge chains are present before Vaultwarden startup"
    )
    startup = next(
        task
        for task in tasks
        if task.get("name") == "Start the Vaultwarden stack and recover Docker bridge-chain failures"
    )

    assert inspect_networks["ansible.builtin.command"]["argv"][:3] == [
        "docker",
        "inspect",
    ...
    assert (
        "vaultwarden_container_networks.stdout"
        in recreate_fact["ansible.builtin.set_fact"]["vaultwarden_force_recreate"]
    )
    assert (
        "vaultwarden_container_ports.stdout" in recreate_fact["ansible.builtin.set_fact"]["vaultwarden_force_recreate"]
    )
    ...
    rescue_fact = next(
        ...
        if task.get("name") == "Flag Docker bridge-chain failures during Vaultwarden startup"
    )
    network_reset = next(
        task
        for task in startup["rescue"]
    ...
    retry_task = next(
        task
        for task in startup["rescue"]
        if task.get("name") == "Retry Vaultwarden startup after Docker bridge-chain recovery"
    )

    assert start_task["ansible.builtin.command"]["argv"] == "{{ vaultwarden_compose_up_argv }}"
    assert (
        "No chain/target/match by that name"
        in rescue_fact["ansible.builtin.set_fact"]["vaultwarden_docker_bridge_chain_missing"]
    )
    assert (
...
def test_verify_checks_controller_liveness_and_local_bootstrap_state() -> None:
    ...
    resolution_task = next(
        task
        for task in verify
        if task.get("name") == "Check whether the Vaultwarden controller hostname resolves locally"
    )
    probe_task = next(task for task in verify if task.get("name") == "Select the Vaultwarden controller probe target")
    bind_task = next(
        task
        for task in verify
        if task.get("name") == "Discover the local Tailscale IPv4 address for fallback controller probes"
    )
    controller_task = next(
        task for task in verify if task.get("name") == "Verify Vaultwarden is reachable through the controller URL"
    )
    fallback_task = next(
        task
        for task in verify
        if task.get("name") == "Verify Vaultwarden is reachable through the controller proxy fallback"
    )
    bootstrap_stat = next(
        task for task in verify if task.get("name") == "Assert the Vaultwarden bootstrap state file exists locally"
    )
    assert (
        resolution_task["ansible.builtin.command"]["argv"][-1]
        == "{{ vaultwarden_controller_url | urlsplit('hostname') }}"
    )
    assert (
        "vaultwarden_service_topology.dns.target"
        in probe_task["ansible.builtin.set_fact"]["vaultwarden_controller_probe_url"]
    )
    assert (
        "vaultwarden_controller_url | urlsplit('port')"
        in probe_task["ansible.builtin.set_fact"]["vaultwarden_controller_probe_url"]
    )
    ...
    assert (
        '--interface "{{ vaultwarden_controller_probe_bind_address.stdout | trim }}"'
        in fallback_task["ansible.builtin.shell"]
    )
    assert (
        "--header 'Host: {{ vaultwarden_controller_url | urlsplit('hostname') }}'"
        in fallback_task["ansible.builtin.shell"]
    )
