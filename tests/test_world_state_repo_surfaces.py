...
def test_windmill_runtime_templates_export_world_state_and_nats_env() -> None:
    env_template = (
        REPO_ROOT / "collections/ansible_collections/lv3/platform/roles/windmill_runtime/templates/windmill.env.j2"
    ).read_text()
    env_ctmpl = (
        REPO_ROOT / "collections/ansible_collections/lv3/platform/roles/windmill_runtime/templates/windmill.env.ctmpl.j2"
    ).read_text()
