...
def test_plausible_runtime_defaults_reference_service_topology_images_and_local_secrets() -> None:
    ...
    assert (
        ...
        == "{{ platform_service_topology | platform_service_port('plausible', 'internal') }}"
    )
    assert (
        defaults["plausible_internal_base_url"]
        == "{{ platform_service_topology | platform_service_url('plausible', 'internal') }}"
    )
    ...
    assert defaults["plausible_secret_key_base_local_file"] == "{{ plausible_local_artifact_dir }}/secret-key-base.txt"
    assert (
        defaults["plausible_bootstrap_user_password_local_file"]
        == "{{ plausible_local_artifact_dir }}/bootstrap-user-password.txt"
    )
    ...
    assert (
        ...
        == "{{ hostvars[platform_topology_host].plausible_site_registrations | default([]) }}"
    )
...
def test_plausible_runtime_tasks_manage_openbao_compose_and_port_recovery() -> None:
    ...
    converge_task = next(task for task in tasks if task.get("name") == "Converge the Plausible compose stack")
    ...
    assert converge_task["vars"]["common_docker_compose_converge_compose_file"] == "{{ plausible_compose_file }}"
    assert converge_task["vars"]["common_docker_compose_converge_site_dir"] == "{{ plausible_site_dir }}"
    assert converge_task["vars"]["common_docker_compose_converge_service_name"] == "plausible"
    assert converge_task["vars"]["common_docker_compose_converge_force_recreate_entire_stack"] is True
    assert converge_task["vars"]["common_docker_compose_converge_health_port"] == "{{ plausible_internal_port }}"
    assert (
        converge_task["vars"]["common_docker_compose_converge_health_url"]
        == "{{ plausible_internal_base_url }}/api/health"
    )
...
def test_plausible_verify_task_checks_tracker_and_synthetic_event_ingestion() -> None:
    ...
    tracker_task = next(
        task for task in tasks if task.get("name") == "Verify the Plausible tracker script responds locally"
    )
...
def test_plausible_runtime_templates_render_public_urls_and_repo_managed_site_checks() -> None:
    ...
    assert (
        "DATABASE_URL=postgres://{{ plausible_database_user }}:{{ plausible_database_password | urlencode }}@plausible-db:5432/{{ plausible_database_name }}"
        in env_template
    )
