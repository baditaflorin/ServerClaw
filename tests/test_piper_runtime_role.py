...
def test_main_tasks_render_build_wait_and_verify_piper() -> None:
    ...
    assert "Build the Piper runtime image" in names
    ...
    assert "Start the Piper runtime and recover Docker nat-chain failures" in names
    ...
    startup_block = next(task for task in tasks if task["name"] == "Start the Piper runtime and recover Docker nat-chain failures")
    rescue_names = [task["name"] for task in startup_block["rescue"]]
    assert "Restart Docker to restore nat chain before retrying Piper startup" in rescue_names
    assert "Retry Piper startup after Docker nat-chain recovery" in rescue_names
...
def test_service_script_and_catalogs_capture_the_private_tts_contract() -> None:
    ...
    assert service["internal_url"] == "http://10.10.10.20:8099"
