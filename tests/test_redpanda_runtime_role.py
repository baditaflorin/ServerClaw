...
def test_main_tasks_prepare_users_topics_and_schema() -> None:
    ...
    assert "redpanda_container_inspect.rc == 0" in recovery_fact["ansible.builtin.set_fact"]["redpanda_recovery_required"]
    assert "redpanda_container_inspect.stdout != 'healthy'" in recovery_fact["ansible.builtin.set_fact"]["redpanda_recovery_required"]
    ...
    reset_task = next(task for task in tasks if task["name"] == "Reset the Redpanda compose stack before recovery cleanup")
    ...
    clear_task = next(task for task in tasks if task["name"] == "Clear the Redpanda crash-tracker startup log before restart")
    ...
    storage_get = next(task for task in tasks if task["name"] == "Read the active Redpanda minimum free storage cluster setting")
    assert storage_get["ansible.builtin.command"]["argv"][:7] == ["docker", "exec", "{{ redpanda_container_name }}", "rpk", "cluster", "config", "get"]
    assert storage_get["changed_when"] is False

    storage_set = next(task for task in tasks if task["name"] == "Reconcile the Redpanda minimum free storage cluster setting")
    assert storage_set["ansible.builtin.command"]["argv"][:7] == ["docker", "exec", "{{ redpanda_container_name }}", "rpk", "cluster", "config", "set"]
    ...
    storage_confirm = next(task for task in tasks if task["name"] == "Confirm the Redpanda minimum free storage cluster setting")
...
def test_verify_tasks_exercise_http_proxy_and_schema_registry() -> None:
    ...
    produce_assert = next(task for task in tasks if task["name"] == "Assert the Redpanda HTTP Proxy smoke produce request succeeded")
    ...
    assert read_task["ansible.builtin.uri"]["url"] == "{{ redpanda_http_proxy_url }}/topics/{{ redpanda_smoke_topic }}/partitions/0/records?offset=0&timeout=3000&max_bytes=1048576"
    ...
    assert payload_task["ansible.builtin.set_fact"]["redpanda_records_payload"] == "{{ redpanda_records_result.content | default('[]', true) | from_json }}"
