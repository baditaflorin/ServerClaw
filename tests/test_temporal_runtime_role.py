...
def test_temporal_runtime_defaults_use_loopback_ports_and_private_namespace() -> None:
    ...
    assert (
        defaults["temporal_admin_tools_image"]
        == "{{ container_image_catalog.images.temporal_admin_tools_runtime.ref }}"
    )
...
def test_temporal_runtime_bootstraps_openbao_env_and_namespace() -> None:
    ...
    openbao_helper = next(
        task for task in tasks if task.get("name") == "Prepare OpenBao agent runtime secret injection for Temporal"
    )
    pull_task = next(task for task in tasks if task.get("name") == "Pull the Temporal compose images")
    recreate_flag_task = next(
        task for task in tasks if task.get("name") == "Record whether the Temporal runtime stack must be recreated"
    )
    ...
    force_recreate_up_task = next(
        task
        for task in tasks
        if task.get("name") == "Converge the Temporal runtime stack with forced recreation when rendered inputs changed"
    )
    up_task = next(
        task
        for task in tasks
        if task.get("name") == "Converge the Temporal runtime stack"
        and "when" in task
        and "temporal_runtime_force_recreate" in task["when"]
    )
    cluster_ready_task = next(
        task
        for task in tasks
        if task.get("name") == "Wait for the Temporal cluster to become ready for namespace administration"
    )
    namespace_check_task = next(
        task for task in tasks if task.get("name") == "Check whether the Temporal namespace already exists"
    )
    ...
    namespace_retention_decision_task = next(
        task
        for task in tasks
        if task.get("name") == "Decide whether the Temporal namespace retention window needs reconciliation"
    )
    namespace_create_task = next(
        task for task in tasks if task.get("name") == "Create the repo-managed Temporal namespace when missing"
    )
    namespace_update_task = next(
        task for task in tasks if task.get("name") == "Reconcile the Temporal namespace retention window"
    )
    namespace_report_task = next(
        task for task in tasks if task.get("name") == "Mirror the Temporal namespace report to the control machine"
    )

    runtime_secret_payload = secret_payload_task["ansible.builtin.set_fact"]["temporal_runtime_secret_payload"]
    assert (
        runtime_secret_payload["TEMPORAL_DB_PASSWORD"]
    ...
    assert (
        ...
        in namespace_retention_task["ansible.builtin.set_fact"]["temporal_namespace_retention_observed"]
    )
    assert (
        "temporal_namespace_retention_observed"
        in namespace_retention_decision_task["ansible.builtin.set_fact"]["temporal_namespace_needs_retention_update"]
    )
...
def test_temporal_verify_task_checks_ui_cluster_health_and_namespace() -> None:
    ...
    ui_task = next(
        task
        for task in tasks
        if task.get("name") == "Verify the Temporal UI responds on the loopback diagnostic endpoint"
    )
    cluster_task = next(
        task
        for task in tasks
        if task.get("name") == "Verify Temporal cluster health through the repo-managed admin-tools profile"
    )
    namespace_task = next(
        task for task in tasks if task.get("name") == "Verify the repo-managed Temporal namespace exists"
    )
    assert_task = next(
        task
        for task in tasks
        if task.get("name") == "Assert Temporal UI, cluster health, and namespace verification are working"
    )
