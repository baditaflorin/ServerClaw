...
def test_openbao_postgres_dynamic_credential_verification_skips_become_on_delegated_uri_calls() -> None:
    ...
    atlas_login_task = next(task for task in verify_play["tasks"] if task.get("name") == "Login to OpenBao as the Atlas AppRole")
    atlas_credential_task = next(
        task for task in verify_play["tasks"] if task.get("name") == "Request an Atlas PostgreSQL schema-inspection credential"
    )
...
def test_openbao_playbook_reestablishes_unsealed_state_before_postgres_end_to_end_verification() -> None:
    plays = yaml.safe_load(PLAYBOOK_PATH.read_text(encoding="utf-8"))
    unseal_play = next(
        play for play in plays if play.get("name") == "Ensure OpenBao remains unsealed before PostgreSQL end-to-end verification"
    )
    ensure_task = next(
        task
