...
COLLECTION_PLAYBOOK = (
    REPO_ROOT / "collections" / "ansible_collections" / "lv3" / "platform" / "playbooks" / "woodpecker.yml"
)
...
def test_woodpecker_root_playbook_bootstraps_controller_artifacts_and_seed_repo() -> None:
    ...
    public_health_task = next(
        ...
        if task["name"] == "Wait for the public Woodpecker health endpoint to answer"
    )
    ...
    task_argvs = [
        task["ansible.builtin.command"]["argv"] for task in bootstrap_play["tasks"] if "ansible.builtin.command" in task
    ]
