...
def test_flagsmith_runtime_tasks_manage_openbao_seed_and_environment_key_flow() -> None:
    ...
    openbao_template_before = next(
        task for task in tasks if task["name"] == "Check whether the Flagsmith OpenBao agent runtime env template already exists"
    )
    openbao_helper = next(
    ...
    openbao_template_changed = next(
        task for task in tasks if task["name"] == "Record whether the Flagsmith OpenBao agent runtime env template changed"
    )
    pull_task = next(task for task in tasks if task["name"] == "Pull the Flagsmith images")
    force_recreate_decision = next(
        task for task in tasks if task["name"] == "Decide whether the Flagsmith compose services require forced recreation"
    )
    build_startup_command = next(task for task in tasks if task["name"] == "Build the Flagsmith compose startup command")
    startup = next(
    ...
    admin_password_task = next(
        task for task in tasks if task["name"] == "Ensure the Flagsmith bootstrap admin password matches the managed secret"
    )
    ...
    assert openbao_template_before["ansible.builtin.stat"]["path"] == "{{ flagsmith_openbao_agent_dir }}/runtime.env.ctmpl"
    ...
    assert openbao_template_after["ansible.builtin.stat"]["path"] == "{{ flagsmith_openbao_agent_dir }}/runtime.env.ctmpl"
    assert "flagsmith_openbao_agent_template_before" in openbao_template_changed["ansible.builtin.set_fact"][
        "flagsmith_openbao_agent_template_changed"
    ]
    assert pull_task["ansible.builtin.command"]["argv"][-1] == "pull"
    assert "flagsmith_openbao_agent_template_changed" in force_recreate_decision["ansible.builtin.set_fact"][
        "flagsmith_force_recreate_required"
    ]
    assert "flagsmith_env_template.changed" in force_recreate_decision["ansible.builtin.set_fact"][
        "flagsmith_force_recreate_required"
    ]
    ...
    assert "changed and user.save(update_fields=[\"password\"])" in admin_password_task["ansible.builtin.command"]["argv"][-1]
...
def test_flagsmith_runtime_recovers_bridge_chain_and_stale_compose_network_failures() -> None:
    ...
    reset_task = next(
        task for task in startup["rescue"] if task["name"] == "Reset stale Flagsmith compose resources after startup failure"
    )
    restart_task = next(
        task
        for task in startup["rescue"]
    ...
    retry_task = next(
        task for task in startup["rescue"] if task["name"] == "Retry Flagsmith startup after resetting stale compose resources"
    )

    assert start_task["ansible.builtin.command"]["argv"] == "{{ flagsmith_compose_up_argv }}"
    assert "No chain/target/match by that name" in recovery_fact["ansible.builtin.set_fact"][
        "flagsmith_docker_bridge_chain_missing"
    ]
    assert "Unable to enable ACCEPT OUTGOING rule" in recovery_fact["ansible.builtin.set_fact"][
        "flagsmith_docker_bridge_chain_missing"
    ]
    assert "Unable to enable DNAT rule" in recovery_fact["ansible.builtin.set_fact"][
        "flagsmith_docker_bridge_chain_missing"
    ]
...
def test_flagsmith_verify_tasks_cover_local_health_task_processor_and_seed_state() -> None:
    ...
    assert_task = next(task for task in tasks if task["name"] == "Assert the local Flagsmith verification results are healthy")
...
def test_flagsmith_templates_bind_private_port_and_render_public_domain() -> None:
    ...
    assert "DATABASE_URL=[[ with secret \"kv/data/{{ flagsmith_openbao_secret_path }}\" ]][[ .Data.data.DATABASE_URL ]][[ end ]]" in openbao_env_template
    assert "DJANGO_SECRET_KEY=[[ with secret \"kv/data/{{ flagsmith_openbao_secret_path }}\" ]][[ .Data.data.DJANGO_SECRET_KEY ]][[ end ]]" in openbao_env_template
