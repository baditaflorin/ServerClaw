...
def test_reaper_treats_null_repo_path_as_default(monkeypatch) -> None:
    monkeypatch.setattr(fixture_expiry_reaper, "DEFAULT_REPO_PATH", "/srv/proxmox_florin_server")
    monkeypatch.setattr(
        fixture_expiry_reaper,
    ...
    payload = fixture_expiry_reaper.main(repo_path=None)
    assert payload["status"] == "blocked"
...
def test_reaper_hydrates_proxmox_env_from_pid1(monkeypatch, tmp_path: Path) -> None:
    ...
    fixture_expiry_reaper._hydrate_proxmox_env_from_pid1(environ_path)
...
def test_reaper_hydrates_proxmox_env_from_runtime_env(monkeypatch, tmp_path: Path) -> None:
    ...
    fixture_expiry_reaper._hydrate_proxmox_env_from_runtime_env(runtime_env_path)
...
def test_reaper_resolves_proxmox_api_credentials_from_env(monkeypatch) -> None:
    ...
    assert fixture_expiry_reaper._resolve_proxmox_api_credentials() == (
...
def test_reaper_reads_proxmox_api_credentials_from_runtime_env(tmp_path: Path) -> None:
    ...
    assert fixture_expiry_reaper._read_proxmox_credentials_from_runtime_env(runtime_env_path) == (
        "https://proxmox.example.invalid:8006/api2/json",
        "lv3-automation@pve!primary=secret-token",
    )
...
def test_reaper_prefers_repo_local_proxmox_payload_when_runtime_env_is_missing(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(fixture_expiry_reaper, "_load_fixture_manager", lambda repo_root: FixtureManagerStub)
    monkeypatch.setattr(fixture_expiry_reaper, "_hydrate_proxmox_env_from_pid1", lambda: None)
    monkeypatch.setattr(fixture_expiry_reaper, "_hydrate_proxmox_env_from_runtime_env", lambda: None)
    monkeypatch.setattr(fixture_expiry_reaper, "_resolve_proxmox_api_credentials", lambda: None)
