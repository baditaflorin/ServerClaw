...
class DocsSiteTests(unittest.TestCase):
    def test_render_site_writes_expected_pages(self) -> None:
        ...
        try:
            ...
            glossary_md = (temp_dir / "reference" / "glossary.md").read_text(encoding="utf-8")
            self.assertIn("# Glossary", glossary_md)
            self.assertIn("Runtime assurance", glossary_md)
    ...
    def test_mkdocs_build_uses_global_robots_override(self) -> None:
        ...
        self.assertIn("contextual-help.css", override_template)
        self.assertIn("contextual-help.js", override_template)
        self.assertIn('for="__search"', header_override)
        self.assertIn("Contextual Help", header_override)
