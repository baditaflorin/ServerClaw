...
CHECK_RUNNER_BUILD_NETWORK ?= $(if $(filter docker-build,$(shell hostname -s 2>/dev/null)),host,)
...
.PHONY: doctor doctor-json doctor-strict heal heal-apply
doctor:
...
heal:
	uv run --with pyyaml python $(REPO_ROOT)/scripts/heal.py
...
validate-service-definitions:
	$(REPO_ROOT)/scripts/validate_repo.sh service-definitions
...
##   make diff-services                         # diff example.com vs 0fork.com
##   make diff-services FROM=example.com TO=0fork.com FORMAT=json
...
diff-services:
	python3 $(REPO_ROOT)/scripts/list_services.py --diff $(or $(FROM),example.com) $(or $(TO),0fork.com) $(if $(FORMAT),--format $(FORMAT),)
...
generate-service-definitions:
	uv run --with pyyaml --with jsonschema python $(REPO_ROOT)/scripts/service_definition_catalog.py --write
...
converge-step-ca:
	...
	ANSIBLE_HOST_KEY_CHECKING=False $(ANSIBLE_ENV) $(ANSIBLE_SCOPED_RUN) --playbook $(REPO_ROOT)/playbooks/step-ca.yml --env $(env) -- --private-key $(BOOTSTRAP_KEY) -e proxmox_guest_ssh_connection_mode=proxmox_host_jump $(EXTRA_ARGS)
...
converge-planka:
	ANSIBLE_HOST_KEY_CHECKING=False $(ANSIBLE_ENV) $(ANSIBLE_SCOPED_RUN) --playbook $(REPO_ROOT)/playbooks/planka.yml --env $(env) -- --private-key $(BOOTSTRAP_KEY) -e proxmox_guest_ssh_connection_mode=proxmox_host_jump -e @$(REPO_ROOT)/playbooks/vars/planka.yml $(ANSIBLE_TRACE_ARGS)

converge-focalboard:
	ANSIBLE_HOST_KEY_CHECKING=False $(ANSIBLE_ENV) $(ANSIBLE_SCOPED_RUN) --playbook $(REPO_ROOT)/playbooks/focalboard.yml --env $(env) -- --private-key $(BOOTSTRAP_KEY) -e proxmox_guest_ssh_connection_mode=proxmox_host_jump -e @$(REPO_ROOT)/playbooks/vars/focalboard.yml $(ANSIBLE_TRACE_ARGS)

converge-kan:
	ANSIBLE_HOST_KEY_CHECKING=False $(ANSIBLE_ENV) $(ANSIBLE_SCOPED_RUN) --playbook $(REPO_ROOT)/playbooks/kan.yml --env $(env) -- --private-key $(BOOTSTRAP_KEY) -e proxmox_guest_ssh_connection_mode=proxmox_host_jump -e @$(REPO_ROOT)/playbooks/vars/kan.yml $(ANSIBLE_TRACE_ARGS)

converge-leantime:
	ANSIBLE_HOST_KEY_CHECKING=False $(ANSIBLE_ENV) $(ANSIBLE_SCOPED_RUN) --playbook $(REPO_ROOT)/playbooks/leantime.yml --env $(env) -- --private-key $(BOOTSTRAP_KEY) -e proxmox_guest_ssh_connection_mode=proxmox_host_jump -e @$(REPO_ROOT)/playbooks/vars/leantime.yml $(ANSIBLE_TRACE_ARGS)

converge-openproject:
	ANSIBLE_HOST_KEY_CHECKING=False $(ANSIBLE_ENV) $(ANSIBLE_SCOPED_RUN) --playbook $(REPO_ROOT)/playbooks/openproject.yml --env $(env) -- --private-key $(BOOTSTRAP_KEY) -e proxmox_guest_ssh_connection_mode=proxmox_host_jump -e @$(REPO_ROOT)/playbooks/vars/openproject.yml $(ANSIBLE_TRACE_ARGS)

converge-taiga:
	ANSIBLE_HOST_KEY_CHECKING=False $(ANSIBLE_ENV) $(ANSIBLE_SCOPED_RUN) --playbook $(REPO_ROOT)/playbooks/taiga.yml --env $(env) -- --private-key $(BOOTSTRAP_KEY) -e proxmox_guest_ssh_connection_mode=proxmox_host_jump -e @$(REPO_ROOT)/playbooks/vars/taiga.yml $(ANSIBLE_TRACE_ARGS)

converge-tiki:
	ANSIBLE_HOST_KEY_CHECKING=False $(ANSIBLE_ENV) $(ANSIBLE_SCOPED_RUN) --playbook $(REPO_ROOT)/playbooks/tiki.yml --env $(env) -- --private-key $(BOOTSTRAP_KEY) -e proxmox_guest_ssh_connection_mode=proxmox_host_jump -e @$(REPO_ROOT)/playbooks/vars/tiki.yml $(ANSIBLE_TRACE_ARGS)

converge-huly:
	ANSIBLE_HOST_KEY_CHECKING=False $(ANSIBLE_ENV) $(ANSIBLE_SCOPED_RUN) --playbook $(REPO_ROOT)/playbooks/huly.yml --env $(env) -- --private-key $(BOOTSTRAP_KEY) -e proxmox_guest_ssh_connection_mode=proxmox_host_jump -e @$(REPO_ROOT)/playbooks/vars/huly.yml $(ANSIBLE_TRACE_ARGS)

converge-flowinquiry:
	ANSIBLE_HOST_KEY_CHECKING=False $(ANSIBLE_ENV) $(ANSIBLE_SCOPED_RUN) --playbook $(REPO_ROOT)/playbooks/flowinquiry.yml --env $(env) -- --private-key $(BOOTSTRAP_KEY) -e proxmox_guest_ssh_connection_mode=proxmox_host_jump -e @$(REPO_ROOT)/playbooks/vars/flowinquiry.yml $(ANSIBLE_TRACE_ARGS)
...
apply-gate-tools:
	$(MAKE) live-apply-service service=build-server-gate-tools env=$(if $(strip $(env)),$(env),production) EXTRA_ARGS='$(EXTRA_ARGS)'
...
converge-site-parallel: ## Parallel site convergence — dependency-wave execution (~40-50% faster)
	...
	ANSIBLE_OVERLAY_EXTRA="$(ANSIBLE_OVERLAY_EXTRA)" \
	ANSIBLE_TRACE_ARGS="$(ANSIBLE_TRACE_ARGS)" \
	EXTRA_ARGS="$(EXTRA_ARGS)" \
	uv run --with pyyaml python $(REPO_ROOT)/scripts/converge_parallel.py
