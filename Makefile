...
.PHONY: syntax-check syntax-check-monitoring syntax-check-uptime-kuma install-proxmox configure-network configure-ingress configure-edge-publication configure-tailscale provision-guests harden-access harden-guest-access harden-security provision-api-access converge-monitoring deploy-uptime-kuma configure-backups start-workstream
