...
capacity-report:
	uv run --with pyyaml python $(REPO_ROOT)/scripts/capacity_report.py --model $(REPO_ROOT)/config/capacity-model.json $(if $(filter true,$(NO_LIVE_METRICS)),--no-live-metrics,) $(if $(FORMAT),--format "$(FORMAT)",) $(if $(filter true,$(CHECK_GATE)),--check-gate,) $(foreach change,$(PROPOSED_CHANGE),--proposed-change "$(change)")

weekly-capacity-report:
	PYTHONPATH=$(REPO_ROOT)/scripts uv run --with pyyaml python $(REPO_ROOT)/config/windmill/scripts/weekly-capacity-report.py --repo-path $(REPO_ROOT) $(if $(filter true,$(NO_LIVE_METRICS)),--no-live-metrics,)
...
scheduler-watchdog-loop:
	PYTHONPATH=$(REPO_ROOT) uv run --with pyyaml python $(REPO_ROOT)/windmill/scheduler/watchdog-loop.py --repo-path $(REPO_ROOT)
...
converge-gitea:
	...
	ANSIBLE_HOST_KEY_CHECKING=False $(ANSIBLE_ENV) ansible-playbook -i $(ANSIBLE_INVENTORY) $(REPO_ROOT)/playbooks/gitea.yml --private-key $(BOOTSTRAP_KEY) -e proxmox_guest_ssh_connection_mode=proxmox_host_jump
