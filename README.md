...
SSO (Authentik), secrets (OpenBao), databases (PostgreSQL), reverse proxy (Nginx),
...
Proxmox VE Host (Debian 13, bare metal)
  |
  +-- nginx-edge        (edge VM)      Reverse proxy, TLS termination
  +-- docker-runtime    (runtime VM)   Primary application runtime
  +-- docker-build      (build VM)     CI/CD build server
  +-- monitoring        (monitoring)   Grafana, Prometheus, Alertmanager
  +-- postgres          (database VM)  Shared PostgreSQL 16
  +-- backup            (backup VM)    Proxmox Backup Server
  +-- runtime-control   (control VM)   API gateway, agent tools
  +-- runtime-ai        (AI runtime)   GPU workloads (Ollama, inference)
...
| `make bootstrap-minimal` | Bootstrap critical path only (PG + Authentik + Nginx + OpenBao) |
