...
def main(
    ...
    try:
        ...
        if not _requires_uv_runtime(exc):
            raise
        return _run_via_uv(
            repo_root / "config" / "windmill" / "scripts" / "graph" / "import-from-catalog.py",
            repo_root,
            dsn,
            world_state_dsn,
        )
    result["source"] = "catalog_and_manifest"
    return result
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Rebuild the ADR 0117 graph from repo metadata.")
